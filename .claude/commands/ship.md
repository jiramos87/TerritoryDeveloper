---
description: Single-task standalone ship pipeline. 4 mechanical steps — author §Plan Digest → implement → verify-loop (cap=2) → close (DB status walk to archived). Standalone tasks only; stage-attached tasks must use /ship-stage. No code review. No audit. No commit.
argument-hint: "{ISSUE_ID} (e.g. TECH-42, BUG-17, FEAT-9)"
---

# /ship — single-task standalone pipeline

Drive `$ARGUMENTS` through 4 mechanical steps via the [`ship`](../agents/ship.md) subagent: **author** §Plan Digest → **implement** → **verify-loop** (`MAX_ITERATIONS=2`) → **close** (DB status walk).
