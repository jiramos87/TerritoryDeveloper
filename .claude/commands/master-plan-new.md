...
> 3. Scope header + dashboard description — Author header block verbatim shape: Status, Scope, Exploration source + sections, Locked decisions, Hierarchy rules pointer, Read-first list (invariants by number from Phase 2, scope-boundary doc if provided). Also author short product `description` (≤200 char soft target) — replaces preamble as dashboard subtitle, required for new plans.
...
> 7. Persist (DB MCP) — `master_plan_insert({slug, title, preamble, description})` → seeds row + preamble + dashboard `description` (≤200 char product overview, required). Per Stage authored: `stage_insert({slug, stage_id, title, body, objective, exit_criteria})`. `master_plan_change_log_append({slug, kind: "plan_authored", body})` → audit row.
