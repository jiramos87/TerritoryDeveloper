...
argument-hint: "{slug} Stage {X.Y} [--force-model {model}] [--no-resume]"
---

# /ship-cycle — Stage-atomic full ship: one inference body emits ALL tasks of one Stage with boundary markers (Pass A) AND drives verify-loop + verified→done flips + inline closeout + single stage commit + cron_stage_verification_flip_enqueue (Pass B). Sole stage-driver in the new chain (design-explore → ship-plan → ship-cycle → ship-final). Falls back to /ship-stage-main-session legacy adapter only when batch exceeds token cap.
