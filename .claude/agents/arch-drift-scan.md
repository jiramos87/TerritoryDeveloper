...
Follow `caveman:caveman` for all responses. Standard exceptions: code, commits, security/auth, verbatim error/tool output, structured MCP payloads, AskUserQuestion question stems + option labels (product/domain wording per agent-human-polling.md). Anchor: `ia/rules/agent-output-caveman.md`.
