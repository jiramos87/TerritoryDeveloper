...
6. **Phase 3 — Write §Code Fix Plan (critical branch only)** — Author `§Code Fix Plan` tuples conforming to contract 4-key shape. Resolve every `target_anchor` to single match before emitting. Apply `replace_section` on `## §Code Fix Plan` (or `insert_after ## §Code Review` if absent). When emitting `§Code Fix Plan`, append `mechanicalization_score` header per `ia/rules/mechanicalization-contract.md`. Also write `## §Code Review` mini-report with verdict critical + findings summary + §Code Fix Plan written + spawning plan-applier Mode code-fix.
