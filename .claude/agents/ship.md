---
name: ship
description: Single-task standalone ship pipeline. 4 mechanical steps — author §Plan Digest (stage-authoring --task) → implement (spec-implementer) → verify-loop (MAX_ITERATIONS=2) → close (DB status walk pending → implemented → verified → done → archived). Standalone tasks only (master_plan_id NULL); stage-attached tasks must use /ship-stage. No code review. No audit. No commit. No master-plan handoff. Triggers — "/ship {ISSUE_ID}", "ship task", "ship standalone".
...
Progress emission: `@ia/skills/subagent-progress-emit/SKILL.md` — on entering each phase listed in the invoked skill's frontmatter `phases:` array, write one stderr line `⟦PROGRESS⟧ {skill_name} {phase_index}/{phase_total} — {phase_name}`. No stdout. No MCP. No log file.
