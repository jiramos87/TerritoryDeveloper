...
description: Use to bulk-author §Plan Digest direct across ALL N filed Task spec stubs of one Stage in a single Opus pass + persist per-Task body to DB via task_spec_section_write MCP. Triggers — "/stage-authoring {ORCHESTRATOR_SPEC} {STAGE_ID}", "stage authoring", "stage-scoped digest". One Opus bulk pass: §Goal / §Acceptance / §Test Blueprint / §Examples / sequential Mechanical Steps with Edits + Gate + STOP + MCP hints + invariant_touchpoints + validator_gate + optional Scene Wiring step. Absorbs canonical-term fold (glossary + retired-surface tombstone + template-section allowlist + cross-ref task-id resolver). Self-lints via plan_digest_lint (cap=1 retry per Task). Mechanicalization preflight via mechanicalization_preflight_lint per Task (advisory hatch when only `picks` field fails AND lint passed AND no missing-path findings). Persists per-Task body via task_spec_section_write MCP — DB sole source of truth (no filesystem mirror). Does NOT write code, flip Task Status, commit, edit glossary, compile aggregate `docs/implementation/{slug}-stage-{STAGE_ID}-plan.md`, or fall back to filesystem-only on db_unavailable.
