...
description: Use to author `ia/projects/{slug}-master-plan.md` orchestrator doc from an exploration doc that already carries a persisted `## Design Expansion` block (or semantic equivalent). Triggers — "/master-plan-new {path}", "turn expanded design into master plan", "create orchestrator from exploration", "author master plan from design expansion", "new multi-step plan from docs/{slug}.md". Decomposes Implementation Points into step > stage > phase > task — ALL steps fully decomposed (no lazy materialization / skeletons). Tasks seeded `_pending_` — does NOT create BACKLOG rows (that is `stage-file`) and does NOT invoke `project-new`. Orchestrator doc is permanent (never closeable via `/closeout`).
