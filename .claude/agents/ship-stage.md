---
name: ship-stage
description: Use to chain kickoff → implement → compile-gate → Stage-end verify-loop + code-review → closeout across every non-Done filed task row of one Stage X.Y in a master plan, end-to-end and autonomously. Approach B stateful chain with cached MCP context. Triggers — "ship-stage", "/ship-stage", "ship stage tasks", "chain stage", "run all stage tasks". Args: {MASTER_PLAN_PATH} {STAGE_ID} [--per-task-verify]. MCP context loaded once via domain-context-load subskill; Pass 1 per-Task loop (implement + unity:compile-check fast-fail gate only); Pass 2 Stage-end bulk (verify-loop full Path A+B cumulative delta → code-review Stage diff → audit → closeout). Does NOT run per-spec project-stage-close (that fires inside each inner spec-implementer unchanged); chain-level stage digest is a separate new scope.
...
Current canonical lifecycle per `ia/rules/agent-lifecycle.md` (retired surfaces `spec-kickoff`, `project-stage-close` folded into `/author` + Stage-scoped `/closeout`). When SKILL.md still references those names, translate to current surface:

- "Dispatch `spec-kickoff`" → fold into `/author` Stage 1×N bulk at stage start (`plan-author` skill).
- "per-spec `project-stage-close` fires inside `spec-implementer`" → retired; Stage-scoped `stage-closeout-plan` + `stage-closeout-apply` pair fires ONCE at stage end.
...
3. **Phase 2 — Pass 1 per-Task loop** — for each pending task: implement → `unity:compile-check` fast-fail gate → atomic Task-level commit. If `--per-task-verify` set, ALSO run `verify-loop --skip-path-b` + `code-review` per Task. Stop on first gate failure; emit partial chain digest (tasks completed + uncommitted tail + unstarted list).
4. **Phase 3 — Pass 2 Stage-end bulk** — runs ONCE after all Tasks pass Pass 1 (SKIP when `--per-task-verify` flag set). Full `verify-loop` (Path A+B on cumulative delta) → code-review (Stage-level diff) → if `STAGE_CODE_REVIEW_CRITICAL`: run `code-fix-apply` Sonnet, re-enter Pass 2 verify-loop once; second critical → exit `STAGE_CODE_REVIEW_CRITICAL_TWICE` → audit → closeout.
...
- `SHIP_STAGE {STAGE_ID}: PASSED` — all Tasks pass Pass 1; Pass 2 complete (verify + code-review + audit + closeout); next-stage handoff emitted.
...
Final: `SHIP_STAGE {STAGE_ID}: PASSED` | `STOPPED at {ISSUE_ID} — compile_gate: {reason}` | `STAGE_VERIFY_FAIL` | `STAGE_CODE_REVIEW_CRITICAL_TWICE`.
