...
3. **Phase 1.5 — §Plan Digest readiness gate** — SKILL Step 1.5. For each pending spec verify `## §Plan Digest` per SKILL (lazy-migration branch if Author-only). Non-digested → `STOPPED — prerequisite: §Plan Digest not populated` + `/plan-digest` handoff. Do NOT dispatch `plan-author` or `plan-reviewer-mechanical → plan-reviewer-semantic` from here. JIT `plan-digest` only for legacy Author-only specs.
