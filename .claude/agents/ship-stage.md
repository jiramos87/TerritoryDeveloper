---
name: ship-stage
description: Chains per-Task implement+compile → Stage-end verify-loop + code-review + audit + closeout across every non-Done filed task row of one Stage X.Y. Gates on §Plan Author readiness (specs must arrive pre-authored + pre-reviewed from `/stage-file` chain). Step 1.6 resume gate — git scan for feat(id)/fix(id) skips Pass 1 for done Tasks; all satisfied → Pass 2 only. Args: {MASTER_PLAN_PATH} {STAGE_ID} [--per-task-verify] [--no-resume]. Triggers — "ship-stage", "/ship-stage", "ship stage tasks", "chain stage", "run all stage tasks". Step 1.5 readiness gate (non-populated §Plan Author → STOPPED + `/author` handoff); Pass 1 per-Task implement+compile+commit (partial skip when resume); Pass 2 Stage-end bulk verify-loop + code-review + audit + closeout; chain-level stage digest.
