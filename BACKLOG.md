...
- [ ] **FEAT-50** — **UI** visual polish: aesthetic refinement (**HUD**, panels, **toolbar**, **MainMenu**)
  ...
  - Files: `Assets/` **UI** prefabs and scenes (`MainMenu.unity`, `MainScene.unity`); `.cursor/specs/ui-design-system.md` (**as-built** vs **Target** alignment); [`.cursor/projects/FEAT-50.md`](.cursor/projects/FEAT-50.md) (polish objectives + benchmarks); [`docs/ui-data-dashboard-exploration-FEAT-50.md`](docs/ui-data-dashboard-exploration-FEAT-50.md) (future dashboard prerequisites — out of **FEAT-50** scope); `docs/reports/ui-inventory-as-built-baseline.json` (refresh when **color** / **typography** tokens change)
  - Spec: `.cursor/projects/FEAT-50.md`
  - Spec sections: `.cursor/specs/ui-design-system.md` — **§1** **Foundations**, **§2** components, **§3** patterns, **§4.3** **Canvas**, **§5.2** theme/prefab paths; `.cursor/specs/unity-development-context.md` **§10** when **Editor** export / **Validate UI Theme** runs; `docs/ui-as-built-ui-critique.md` (optional cross-check)
  - Notes: Look-and-feel pass—palette, **typography** rhythm, spacing, icon treatment, contrast, optional motion—**without** changing **simulation** or **grid** rules. **Uses** shipped **UI-as-code** baseline (**`UiTheme`**, prefab **v0**, **`UIManager` partials** — **`ui-design-system.md`** **§5.2**); may extend **`UiTheme`** fields and **`DefaultUiTheme.asset`** values. Normative checklist: **`.cursor/projects/FEAT-50.md`**; future **data-dashboard** prerequisites: **`docs/ui-data-dashboard-exploration-FEAT-50.md`** (out of **FEAT-50** delivery scope).
