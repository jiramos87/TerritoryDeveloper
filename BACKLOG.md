...
- [ ] **TECH-31** — **Agent scenario generator** (**test mode**, **32×32** **test map**, loadable **save**-shaped scenarios, MCP tool)
  ...
  - Notes: **Program tracker** (staged implementation, progress, lessons): [`projects/TECH-31-agent-scenario-generator-program.md`](projects/TECH-31-agent-scenario-generator-program.md) — stages **31a** → **31a2** → **31a3** → **31b**–**31e**; **31a2** (shipped): **glossary** **Agent test mode batch** — **`npm run unity:testmode-batch`**, **`tools/scripts/unity-quit-project.sh`**; [`ARCHITECTURE.md`](ARCHITECTURE.md) **Local verification**; [`tools/fixtures/scenarios/README.md`](tools/fixtures/scenarios/README.md). **31a3** = **`agent-test-mode-verify`** skill orchestration (see [`projects/TECH-31a3-agent-test-mode-verify-skill.md`](projects/TECH-31a3-agent-test-mode-verify-skill.md)); **31d** = **TECH-82** Phase 1 + **test mode** **city history**. **Cursor stub** (Open Questions + aggregate test contracts): `.cursor/projects/TECH-31.md`. **Agent-facing** only: structured intent → **`GameSaveData`-compatible** artifact + stable **scenario id** (recommend **kebab-case**); **test mode** on **32×32**; bounded **simulation** work; export/assert via **UTF**, **`debug_context_bundle`**, golden JSON. **Core builder APIs** + validation (reject or repair per stub **Open Questions**); **territory-ia** MCP in stage **31e**. **Minimal UI** (**TEST-MODE**); gate **test mode** so release builds cannot enable it. **City history**: **TECH-82** Phase 1 when Postgres is configured; else save + bundle. Document **UTF** / **batchmode** / bridge driver matrix for local vs **CI**, max **simulation ticks** for **CI**, and regenerate fixtures when **`GameSaveData`** version changes. Keep expected values consistent across stage **Test contracts**, descriptor `expected`, and **Acceptance** below. Composes with **close-dev-loop**. `docs/agent-tooling-verification-priority-tasks.md` (agent tooling themes).
  - Acceptance: at least one automated Unity run (**Edit Mode** or **Play Mode**) on a clean tree with a committed scenario; documented **test mode** launch (**scenario id** or path, **32×32**); builder docs for **AUTO** and at least one non-**AUTO** pattern; **close-dev-loop** and **TECH-82** **city history** composition documented in spec; **`npm run unity:compile-check`** after tooling changes; **MCP** tool registered and described in **`docs/mcp-ia-server.md`**; spec **§7b** lists chosen driver, args, and **CI** tick bound; optional **BUG-52** **Notes** link when first **AUTO** scenario lands
