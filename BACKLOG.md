...
**Charter:** Reduce **manual Unity Editor** work for **HUD**, **menus**, **panels**, and **toolbars** by making **UI** composable from the **IDE** (Cursor) and **AI agents** — via **reference spec** clarity (`.cursor/specs/ui-design-system.md`), a future **runtime C#** UI kit, **Editor** / **CLI** automation aligned with **Unity**’s standard **Canvas** / **uGUI** / **TextMeshPro** workflows, and **Cursor Skills** + optional **territory-ia** affordances. **First deliverable:** **TECH-68** — document **as-built** (**current**) **UI** in **`ui-design-system.md`** (baseline before target-state refactors). **Exploration workbook:** [`projects/ui-as-code-exploration.md`](projects/ui-as-code-exploration.md).
...
- [ ] **TECH-67** — **UI-as-code program** (umbrella): **IDE**-first **UI** authoring + **`ui-design-system.md`** + runtime kit + agent tooling
  ...
  - Files: `.cursor/specs/ui-design-system.md`; [`projects/ui-as-code-exploration.md`](projects/ui-as-code-exploration.md) (charter + inventory + exploration); `Assets/Scripts/` (runtime **UI** helpers — TBD); `Assets/Scripts/Editor/` (**Editor** automation — TBD); `.cursor/skills/` (TBD); `tools/` (optional **Node** / **Unity** **`batchmode`** — TBD); `tools/mcp-ia-server/` (optional **MCP** tools — TBD)
  ...
  - Acceptance: **§8** in **`.cursor/projects/TECH-67.md`**; **TECH-68** **§8** satisfied (**as-built** reference spec); further **child** rows for runtime kit + **Editor**/**agent** tooling as scoped; workbook kept current
