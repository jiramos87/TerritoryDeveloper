...
- [ ] **BUG-45** — Adjacent water bodies at different surface heights: merge, prefab refresh at intersections, straight slope/cliff transitions
  - Type: bug / polish
  - Files: **`docs/water-junction-merge-implementation-plan.md`** (step-by-step implementation plan), `WaterManager.cs` (`UpdateWaterVisuals` — Pass B after `ApplyMultiBodySurfaceBoundaryNormalization`, before `PlaceWater`), `WaterMap.cs` (new junction merge pass, body bookkeeping), `TerrainManager.cs` (`DetermineWaterShorePrefabs`, `PlaceWaterShore`, `RefreshWaterCascadeCliffs`, shore dirty region), `GridManager.cs` / `Cell` height sync if needed, `ProceduralRiverGenerator.cs` if generator must align contacts with strip width; water / shore / cliff prefabs under `Assets/Prefabs/` (existing diagonals + `cliffWaterSouth` / `cliffWaterEast` per `.cursor/rules/coding-conventions.mdc`)
  - Spec: `.cursor/specs/isometric-geography-system.md` — **§12.7** (Pass A bed alignment + Pass B junction merge), **§5.6.2** (cascades)
  - Notes: **Observed:** Two bodies at **different logical surfaces** meeting on a **cardinal** edge → voids, floating water, broken shores. **Approach (planned):** **Pass A** keeps current bed-only alignment on the **upper** contact (`ApplyMultiBodySurfaceBoundaryNormalization`). **Pass B** extends the **lower** surface into a **parametric strip** (may convert dry / water-shore to open water), assigns **`waterBodyId`** to a body at **`S_low`** without merging two lower bodies that share the same height; **diagonal** upper shores oriented from **contact direction**; **`cliffWaterSouth` / `cliffWaterEast`** on upper faces including diagonal upper shores where needed; **`ΔS > 1`** → longer stacks. **Related:** completed **BUG-42** (cascade pipeline). **Implementation (2026-03-26):** **`RefreshWaterCascadeCliffs`** places cascades on **every** `S_high > S_low` cardinal edge (removed same-surface continuation suppression). **`GetEffectiveHeightsForWaterWaterCliff`** extends foot synthesis when beds coincide at `MIN_HEIGHT`. **`SelectPerpendicularWaterCornerPrefabs`** prefers diagonal slope over Bay when two perpendicular **registered** waters differ in `SurfaceHeight`. **`UpdateWaterVisuals`** runs **`RefreshLakeShoreAfterLakePlacement`** when Pass B merges or depression-fill is on (second Chebyshev ring when Pass B merged). **Lake at surface step:** **`WaterMap.IsLakeSurfaceStepContactForbidden`** — Pass A/B and cascades skip the edge when either body is **`Lake`** (not **`Sea`**). Spec §5.6.2, §12.7 updated. **Lake high vs river low (2026-03-27):** **`WaterMap.ApplyLakeHighToRiverLowContactFallback`** — strips cardinal **Lake** cells touching a lower **`River`**, sets dry rim to logical **`S`**, **`RestoreTerrainForCell`** before **`PlaceWater`** and again after lake-shore refresh (override **`ClampShoreLandHeightsToAdjacentWaterSurface`**). **Do not** mark **Completed** until verified in Unity.
  - Depends on: none
