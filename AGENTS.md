...
| On completion | Lessons-learned migration + spec deletion handled inline by `/ship-stage` Pass B (stage_closeout_apply MCP) |
...
**Multi-stage specs.** Large rewrites declare top-level **stages** with internal **phases**, executed by one fresh agent per stage; `/ship-stage` Pass B closes ALL Task rows of one Stage X.Y in bulk inline (archive N yaml + delete N specs + flip N rows + chain-level digest in one pass via stage_closeout_apply MCP).
...
**Closing issue with project spec.** `/ship-stage {MASTER_PLAN_PATH} {STAGE_ID}` Pass B closes the Stage inline via `stage_closeout_apply` MCP (unified shared migrations + N archive/delete/flip/purge ops applied once + `materialize-backlog.sh` + `validate:dead-project-specs` + chain-level Stage closeout digest). Per-Task closeout surface retired — no per-spec lessons-migration pass.
...
- **Orchestrator:** DB-backed master plan slug `web-platform` (`ia_master_plans` row; render via `mcp__territory-ia__master_plan_render({slug: "web-platform"})`) — permanent, never closeable via `/closeout`.
- **Live dashboard freshness:** `/dashboard` reads master plans from the DB (`ia_master_plans` + `ia_stages`) via the web-platform DB connection — no static `.md` files. Changes via DB MCP tools propagate on next page load.
