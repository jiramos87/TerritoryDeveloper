...
4. **IA persistence (edit durable docs)** — Apply the **IA persistence checklist** using **`router_for_task`** + **`spec_section`** / **`spec_sections`** + **`glossary_discover`** / **`glossary_lookup`** (**English**). Use **`list_rules`** / **`rule_content`** when editing **`.mdc`**.
4b. **Optional (`project-implementation-validation`)** — After step 4 (and **I1** when applicable), consider [`project-implementation-validation`](../project-implementation-validation/SKILL.md) before step 7.
