...
> **Status:** Draft
> **Created:** 2026-04-04
> **Last updated:** 2026-04-06 (kickoff: IA alignment, guardrails, **§7** / **§7b** concrete exits)

> **Parent program:** This row is the umbrella. **First child:** [TECH-68](TECH-68.md) — **as-built** documentation of **`ui-design-system.md`**. Further children (**runtime UI kit**, **Editor** / agent tooling) follow after **TECH-68** **§8**. **Program charter**, **codebase inventory**, and **roadmap** below replace the retired **`projects/ui-as-code-exploration.md`** workbook.
...
1. **As-built spec fidelity:** **`ui-design-system.md`** documents **current** **colors**, **typography**, **spacing**, **layout**, **Canvas** settings, and major **UX** surfaces **across all shipped UI scenes** (**MainMenu**, **city** view, future **Region**, etc.) — **TECH-68**.
...
Umbrella issues stay mostly documentation, but **Phase 3**/**TECH-07** and future children may edit **`UIManager`**, controllers, or scenes. Align with **`.cursor/rules/invariants.mdc`** and project patterns:
...
| 3 | Maintainer | I want **program notes** and **child** specs so phases have clear boundaries. | **TECH-67** **§4.4–§4.9** + **TECH-68** + later **child** rows. |
...
**`ui-design-system.md`** lists foundations and patterns but many rows are still **TBD**; the game already has substantial **uGUI** implementation across **multiple scenes** (**`MainMenu`**, **`MainScene`** / future **`CityScene`**, etc.). **TECH-68** closes the **documentation / reality** gap with **scene-scoped** exports and spec prose. **TECH-07** (**ControlPanel**) applies to the **city** scene **toolbar**, not necessarily **MainMenu**. **Soft** order: **TECH-68** before **TECH-07** for a diffable baseline.
...
| Toolbar / layout debt | **TECH-07** — **ControlPanel**; inventory **§4.4** |
| Prefab / scene introspection | **TECH-33** |
| Editor diagnostics pattern | `.cursor/specs/unity-development-context.md` §10 |
| **Multi-scene UI** | **TECH-68** allowlist + **`scenes[]`** JSON — extend when **`RegionScene`** / **`CityScene`** rename lands |
...
*Migrated from the retired **`projects/ui-as-code-exploration.md`** workbook. **Scene** object names and **Inspector** wiring can drift — verify in **Unity** when implementing **TECH-68** or refactors. Update this subsection when hierarchies or roles change.*
...
**City scene and `ControlPanel`:** Primary layout **`Assets/Scenes/MainScene.unity`** (or future **`CityScene.unity`**). **`ControlPanel`**: construction **toolbar** (demolition, **RCI** **zoning**, utilities, **streets**, forest/environment); wired via **`UIManager`** and **`UnitControllers/*SelectorButton.cs`**. **Target layout:** **TECH-07** + **`ui-design-system.md`** **§3.3**.
...
**Program success checklist** (revisit when closing **TECH-67** or a major child): **TECH-68** **§8** done; foundations reflect **as-built** first; minimal **component set** prefabs or documented variants (**`ui-design-system.md`** **§4.1**); new UI **BACKLOG** rows cite spec sections (**reference spec §5**); **Decision Log** rows here stay current.

### 4.6 Backlog bridge (executable work)

| Workstream | Backlog | Reference spec | Notes |
|------------|---------|----------------|-------|
| **As-built** **UI** documentation | **TECH-68** | **`ui-design-system.md`** **§1–§4**, **§2–§3** | **Multi-scene** export; **glossary** **UI design system (reference spec)** |
| **Toolbar / ControlPanel** | **TECH-07** | **§3.3**, **§1.3**, **§4.3** | **Soft:** after **TECH-68**; **city** scene |
| **Prefab** / scene introspection | **TECH-33** | [`.cursor/projects/TECH-33.md`](TECH-33.md) | Complements **TECH-68** export; does **not** replace **Graphic** / **RectTransform** sampling |
...
| Risk | Mitigation |
|------|------------|
| **`UIManager`** keeps growing | Extract controllers / helpers; **TECH-07** + follow-up **TECH-** rows |
| Scene-only truth | Document **Canvas** paths in **`ui-design-system.md`** + **TECH-68** JSON export |
...
- **TECH-68** does **not** include constructing the whole **UI** from the markdown spec alone; it **documents** and **exports** **as-built** state (see [TECH-68](TECH-68.md) **§5**). **Unity** assets remain authoritative until a **child** issue explicitly introduces procedural construction.
- Confidence in the spec does **not** require “only when we rebuild from the doc.” **Structural** checks (**TECH-68** JSON **`scenes[]`** ↔ spec tables), **git** diffs on all **UI-bearing** `.unity` files / prefabs, and **Play Mode** spot-checks **per scene flow** (menu → load city, etc.) are enough for baseline validation.
...
1. **Export diff after documenting —** Implementer fills **§1.2** from **`UIManager`** + **city** scene and **MainMenu** hierarchy. They run **`Territory Developer → Reports → Export UI Inventory (JSON)`** (per **TECH-68**). If **`scenes[].scene_name == "MainScene"`** (or **`CityScene`** after rename) and node **`Canvas/HUD/MoneyText`** shows `font_size: 20` but the spec table still says `18`, fix the table **or** fix the scene — no procedural **UI** build required. **MainMenu** entries appear under their own **`scenes[]`** block with **`scene_asset_path`** **`Assets/Scenes/MainMenu.unity`**.

2. **TECH-07 toolbar without losing state —** Before reparenting **`ControlPanel`**, commit (or tag) current **`MainScene.unity`** (or **`CityScene.unity`**). **MainMenu** is unaffected by **TECH-07** but should still appear in the **multi-scene** export. Land **TECH-68** **as-built** **§3.3** **Current (legacy)**. Implement **TECH-07** on a branch; keep a duplicate **`ControlPanel_Legacy`** disabled under the **city** **`Canvas`** until **Play Mode** verifies the new sidebar; then remove legacy in a follow-up commit.

3. **Future `RegionScene` —** Add **`Assets/Scenes/RegionScene.unity`** to the **TECH-68** allowlist; extend **`ui-design-system.md`** with a **Region** / **map** **UI** subsection; re-run export so **`scenes[]`** gains a third entry without breaking existing paths.
...
| 2026-04-06 | **Multi-scene** **UI** scope (**MainMenu** + **city** + future **Region** / **`CityScene`** rename) | **TECH-68** JSON and spec must not assume **MainScene** only | Single-scene **UI** program (rejected) |
...
| 2026-04-06 | **project-spec-kickoff** pass — **§1.1** IA, **§2.3** runtime guardrails, concrete **§7**/**§7b** | Align umbrella with **BACKLOG** **Spec sections**, **invariants**, and **TECH-68** phased plan | Keep **§7** as one-line phases only (rejected) |
| 2026-04-06 | **TECH-68** **§7** holds per-phase **deliverables**/**verification**; **TECH-67** **Phase 1** references child spec | Avoid duplicating full checklist in umbrella | Copy entire **TECH-68** **§7** into **TECH-67** (rejected) |
...
- [ ] After edits to **BACKLOG** **`Spec:`** lines or cross-links among **TECH-67** / **TECH-68** / siblings: `npm run validate:dead-project-specs` (repo root).

### Phase 1 — As-built reference spec (**TECH-68**)

Execute [`.cursor/projects/TECH-68.md`](TECH-68.md) **§7** through **§8** (umbrella **Phase 1** = child spec phases **0–5**). **TECH-68** **§7** lists per-phase **deliverables** and **verification** (kickoff-expanded). **Preferred cadence:** **TECH-68** **§1.2** — **Editor** code in one block, **Unity** export gate by a human, then **`ui-design-system.md`** / **§4.4** persistence. Minimum concrete exits:

- [ ] **TECH-68** Phase **0–1:** allowlisted scenes + **Editor** **UI** inventory export (bounded **JSON** with **`scenes[]`**) per **TECH-68** **§5.5** / **§7** Phase 0–1.
- [ ] **TECH-68** Phase **2–3:** **`ui-design-system.md`** **§1**, **§4.3** **Canvas Scaler**, **§2–§3** surfaces populated **as-built**; **§3.3** **Current** vs **Target** explicit for **ControlPanel**.
- [ ] **TECH-68** Phase **4–5** (as scoped): sync **§4.4** here if audit changed hierarchy; `npm run generate:ia-indexes -- --check` after **`ui-design-system.md`** body edits.
- [ ] **Gate:** **TECH-68** **§8** acceptance checklist fully satisfied (umbrella **§8** first bullet).

### Phase 2 — Target layout and migration (**TECH-07** and related)

- [ ] Prefer starting after **TECH-68** **§8** so **`ui-design-system.md`** records **as-built** **toolbar** first (**§5.3** order).
- [ ] Implement **TECH-07** per **BACKLOG**; **Play Mode** verify **§3.5**-class issues (scroll vs **camera**) on **HUD** / **toolbar** / **popups**.
- [ ] Update **`ui-design-system.md`** **§3.3** so **Target** matches shipped layout and **as-built** rows reflect post-merge **reality**.

### Phase 3 — Runtime UI kit (child issue TBD)

- [ ] File a dedicated **BACKLOG** row with **Files**, **Spec:** `.cursor/projects/{ISSUE_ID}.md`, and **Depends on** (expect **soft:** **TECH-68** complete).
- [ ] Land **C#** primitives (panels, buttons, style helpers) under agreed **namespace** / **asmdef**; **§2.3** guardrails; XML docs on new public **API** per **`.cursor/rules/coding-conventions.mdc`**.

### Phase 4 — Editor / agent tooling (child issue TBD)

- [ ] **Editor** menus or **CLI** for scaffold / validate / export — follow **`unity-development-context.md`** **§10** conventions (menu paths, `tools/reports/` fallbacks, optional **Postgres** **Editor export registry** when applicable).
- [ ] **Cursor Skill(s)** + optional **territory-ia** **MCP** tools: register in **`tools/mcp-ia-server`**, update **`docs/mcp-ia-server.md`** + package **README** (**snake_case** tool names); run **`npm run verify`** under **`tools/mcp-ia-server/`** when MCP code ships.

## 7b. Test Contracts

| Acceptance / goal | Check type | Command or artifact | Notes |
|-------------------|------------|---------------------|-------|
| **TECH-67** **§8** bullet 1 (**as-built** baseline) | Deferred to child | Satisfied when **TECH-68** **§8** complete | Use **TECH-68** **§7b** row for **`generate:ia-indexes --check`** |
...
| Runtime **UI** kit (**child** issue) | Manual / UTF | **TBD** | When scaffolding exists; respect **§2.3** |
