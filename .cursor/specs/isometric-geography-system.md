...
6. **`UpdateWaterVisuals` order:** Pass A → Pass B → **`ApplyLakeHighToRiverLowContactFallback`** (when applicable) → **`PlaceWater`** (all water cells) → **`RefreshWaterCascadeCliffs`** → **`RefreshLakeShoreAfterLakePlacement`** when lake depression-fill is enabled, Pass B changed any cell, **or** the lake–river fallback ran (Chebyshev-2 halo when Pass B merged or fallback ran).
