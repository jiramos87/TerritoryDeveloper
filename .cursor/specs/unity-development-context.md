...
| Agent test mode batch report | `tools/reports/agent-testmode-batch-*.json` (gitignored) | **Batchmode** **Editor** via **`npm run unity:testmode-batch`** — **`GameSaveManager.LoadGame`** on a **fixture** path; optional simulation ticks; **`EditorApplication.Exit`** exit code. **glossary** **Agent test mode batch**; [`tools/fixtures/scenarios/README.md`](../../tools/fixtures/scenarios/README.md). Distinct from **IDE agent bridge** exports. |
