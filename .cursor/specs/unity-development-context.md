...
| UI inventory (dev) | `tools/reports/ui-inventory-{UTC-timestamp}.json` | **Edit Mode**; **`artifact`**: `ui_inventory_dev` — bounded **`scenes[]`** (**MainMenu** + **city** scene allowlist), **Canvas** / **RectTransform** / **Graphic** / **Text** / **TMP** sampling; not **Save data** (**`ui-design-system.md`** — **Machine-readable traceability**) |
