-- 0159_master_plan_bundle_apply_design_priority.sql
--
-- Extend master_plan_bundle_apply to write priority + design_id on
-- ia_master_plans and auto-flip the linked ia_plan_designs row to
-- status='consumed' inside the same tx.
--
-- Changes vs mig 0136:
--
--   - Parse `bundle.plan.priority` (default 'P2') and validate against
--     enum P0/P1/P2/P3.
--   - Parse `bundle.plan.design_id` (nullable BIGINT). When present,
--     guard that the row exists FOR UPDATE before proceeding.
--   - All three apply paths (insert_new / backfill_replace / version_bump)
--     write priority + design_id on the ia_master_plans row.
--   - After successful plan-row mutation, if design_id is present and
--     the seed status is not yet 'consumed', flip status='consumed' +
--     bump updated_at. Idempotent (no-op when already consumed).
--   - Return jsonb gains `design_id_linked` (bigint | null) and
--     `design_seed_promoted` (bool) fields.
--
...
DECLARE
  ...
  v_design_promoted     boolean := false;
...
BEGIN
  ...
  -- 1b. Priority (default P2, validate enum).
  v_priority := COALESCE(p_bundle->'plan'->>'priority', 'P2');
  IF v_priority NOT IN ('P0','P1','P2','P3') THEN
    RAISE EXCEPTION
      'master_plan_bundle_apply: invalid priority % (must be P0/P1/P2/P3)',
      v_priority;
  END IF;

  -- 1c. design_id (nullable; FOR UPDATE guard when present).
  v_design_id := NULLIF(p_bundle->'plan'->>'design_id', '')::bigint;
  IF v_design_id IS NOT NULL THEN
    SELECT status INTO v_design_status
      FROM ia_plan_designs
     WHERE id = v_design_id
     FOR UPDATE;
    IF NOT FOUND THEN
      RAISE EXCEPTION
        'master_plan_bundle_apply: design_id=% not found in ia_plan_designs',
        v_design_id;
    END IF;
  END IF;
  ...
  -- 3. Apply plan row mutation per path (priority + design_id everywhere).
  ...
  -- 3.6. Auto-flip seed row to 'consumed' when bundle linked a design_id.
  IF v_design_id IS NOT NULL AND COALESCE(v_design_status, '') <> 'consumed' THEN
    UPDATE ia_plan_designs
       SET status = 'consumed', updated_at = now()
     WHERE id = v_design_id;
    v_design_promoted := true;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'priority',             v_priority,
    'design_id_linked',     v_design_id,
    'design_seed_promoted', v_design_promoted,
...
COMMENT ON FUNCTION master_plan_bundle_apply(jsonb) IS
  'Mig 0159: priority (P0-P3) + design_id FK + auto-flip seed to consumed. Mig 0136: server-side stage body render via format_stage_body(red_stage_proof_block); promote_drift_lint_staged() flips staged→queued in same tx.';
