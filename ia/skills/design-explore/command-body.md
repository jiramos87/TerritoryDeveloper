...
> 2.5. Architecture Decision (DEC-A15 lock) — fires when selected approach touches `arch_surfaces`. Skip-clause: zero arch hits → silent no-op. 4 sequential AskUserQuestion polls (slug → rationale → alternatives → affected `arch_surfaces[]`). MCP writes: `arch_decision_write` (status=active) → `arch_changelog_append` (kind=`design_explore_decision`) → `arch_drift_scan` against open master plans. Drift report appended inline under `### Architecture Decision` block. Stop on any MCP write failure.
