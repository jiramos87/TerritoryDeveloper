...
12. **Phase 10 — Next-stage resolver** — re-call `master_plan_state(slug)`. Sort stages by **numeric tuple `(major, minor)`** parsed from `stage_id` (e.g. `8.1` → `(8,1)`, `19.2` → `(19,2)`); iterate forward starting from first stage with `(major, minor) > current STAGE_ID_DB`. Pick the **first** stage by numeric order matching one of 4 cases — do NOT skip skeletons or pending stages to grab a later filed stage: filed → `/ship-stage`; pending (`_pending_` Issue ids) → `/stage-file`; skeleton (no tasks) → `/stage-decompose`; umbrella-done → `/closeout {UMBRELLA_ISSUE_ID}` (or "umbrella close pending"). `stage-decompose` + `stage-file` are part of the standard incremental flow.
