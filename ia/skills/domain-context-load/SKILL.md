---
purpose: "Run the shared MCP context recipe (glossary_discover → glossary_lookup → router_for_task → spec_sections → invariants_summary) and return a structured payload. Single source for the recipe shared by 8+ lifecycle skills."
audience: agent
...
description: >
  ...
  and returns a structured payload `{glossary_anchors, router_domains, spec_sections, invariants}`.
