...
**Output style — caveman default.** Follow `caveman:caveman` skill rules for the human-readable summary that wraps the Verification block (drop articles/filler/pleasantries/hedging; fragments OK; pattern `[thing] [action] [reason]. [next step].`). Standard exceptions apply: code, commits, security/auth content, verbatim error/tool output, the **structured JSON Verification header itself** (which must parse as JSON), destructive-op confirmations. The directive applies whether the skill is invoked via the `verifier` subagent or directly inline. Project anchor: [`ia/rules/agent-output-caveman.md`](../../rules/agent-output-caveman.md).
