...
| [`code-fix-apply/`](code-fix-apply/SKILL.md) | Sonnet pair-tail (seam #4): applies §Code Fix Plan tuples; re-enters /verify-loop; 1-retry bound | opus-code-review emitted critical §Code Fix Plan | `ia/projects/{ISSUE_ID}.md` with §Code Fix Plan | Tuples applied; verify gate passed; escalates to Opus on second fail | none |
