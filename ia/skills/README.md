...
| [`plan-digest/`](plan-digest/SKILL.md) | Opus bulk: mechanize §Plan Author → §Plan Digest; compile `docs/implementation/{slug}-stage-{id}-plan.md` | `/stage-file` Step 4 or `/plan-digest` standalone; after `plan-author` | N specs with populated §Plan Author | §Plan Digest per spec; aggregate stage doc; `plan_digest_lint` | none |
...
| [`opus-audit/`](opus-audit/SKILL.md) | Stage-scoped bulk audit: one Opus pass writes all N §Audit paragraphs | All Tasks in Stage reach post-verify Green | `ia/projects/{ISSUE_ID}.md` ×N with §Findings; shared Stage MCP bundle | N `§Audit` paragraphs written; feeds `stage-closeout-plan` | none |
...
| [`plan-applier/`](plan-applier/SKILL.md) | Unified Sonnet pair-tail (TECH-506): Modes plan-fix / code-fix / stage-closeout — applies §Plan Fix, §Code Fix Plan, or §Stage Closeout Plan tuples; gates per `plan-apply-pair-contract.md` | Pair-head wrote tuples (`plan-review` / `opus-code-review` / `stage-closeout-plan`) | Target §Plan section + paths in tuples | Tuples applied; per-mode validation gate | none |
<!-- project-spec-kickoff retired per M6 collapse — folded into `plan-author` (Stage 1×N). Tombstone: `_retired/project-spec-kickoff/SKILL.md`. -->
...
Copy-paste stub (no frontmatter): [`ia/templates/project-spec-review-prompt.md`](../templates/project-spec-review-prompt.md). **§Plan Author** (ephemeral) + **§Plan Digest** (canonical executable plan) — **`plan-author/SKILL.md`** then **`plan-digest/SKILL.md`**; **implementation** in **`project-spec-implement/SKILL.md`**; **post-implementation Node checks** in **`project-implementation-validation/SKILL.md`**; **agent test-mode** batch/bridge loop in **`agent-test-mode-verify/SKILL.md`**; optional **Unity** log/screenshot bridge in **`ide-bridge-evidence/SKILL.md`**; **Close Dev Loop** before/after **`debug_context_bundle`** in **`close-dev-loop/SKILL.md`**; **Stage-scoped closeout pair** authoritative in **`stage-closeout-plan/SKILL.md`** + **`plan-applier/SKILL.md`** (Mode stage-closeout); **new issue + spec stub** in **`project-new/SKILL.md`**.
