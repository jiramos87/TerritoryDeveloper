...
| [`project-spec-kickoff/`](project-spec-kickoff/SKILL.md) | Enrich spec before implementation | Spec stub exists; Implementation Plan too vague to execute | `ia/projects/{ISSUE_ID}.md` stub | Enriched spec with filled Implementation Plan | none |
...
| [`project-stage-close/`](project-stage-close/SKILL.md) | Per-stage close for multi-stage specs | Non-final stage verified; ready to advance | Stage phases done + Verification block passed | §7 phase ticks + Decision Log entry + paste-ready handoff | none |
| [`project-spec-close/`](project-spec-close/SKILL.md) | Umbrella close — full issue teardown | All stages verified; issue ready to archive | Verification block passed + spec exists | BACKLOG-ARCHIVE row + lessons migrated to canonical IA | Deletes `ia/projects/{ISSUE_ID}.md`; removes BACKLOG row; purges id refs in durable docs/code |
