...
| [`ship-plan/`](ship-plan/SKILL.md) | DB-backed bulk plan-author: lean YAML → §Plan Digest per Task | `## Design Expansion` + lean YAML frontmatter in exploration doc | `docs/explorations/{slug}.md` with YAML frontmatter | `ia_master_plans` + `ia_stages` + `ia_tasks` rows + §Plan Digest per Task body | none |
...
Copy-paste stub (no frontmatter): [`ia/templates/project-spec-review-prompt.md`](../templates/project-spec-review-prompt.md). **§Plan Digest** (canonical executable plan) authored by **`ship-plan/SKILL.md`** (bulk author: lean YAML → §Plan Digest per Task body, `master_plan_bundle_apply` tx); **implementation** in **`project-spec-implement/SKILL.md`**; **post-implementation Node checks** in **`project-implementation-validation/SKILL.md`**; **agent test-mode** batch/bridge loop in **`agent-test-mode-verify/SKILL.md`**; optional **Unity** log/screenshot bridge in **`ide-bridge-evidence/SKILL.md`**; **Close Dev Loop** before/after **`debug_context_bundle`** in **`close-dev-loop/SKILL.md`**; **Stage closeout** runs inline in **`/ship-stage`** Pass B via `stage_closeout_apply` MCP; **new issue + spec stub** in **`project-new/SKILL.md`**.
