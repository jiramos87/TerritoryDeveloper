...
| J1 | **Postgres** **`ia_project_spec_journal`** ([**glossary** **IA project spec journal**](../../../ia/specs/glossary.md)) | **Verbose** **Decision Log** + **Lessons learned** only — use MCP **`project_spec_journal_persist`** (same `issue_id` / `spec_path` as **`project_spec_closeout_digest`**, optional `git_sha`) **after** normative **G1–I1** edits, **before** deleting the project spec. **CLI:** `npm run db:persist-project-journal` from repo root. **Skip** with one chat line when neither **`DATABASE_URL`** nor [`config/postgres-dev.json`](../../../config/postgres-dev.json) yields a URL (or **CI** skips the file fallback without **`DATABASE_URL`**). **On `db_error`:** do **not** delete `ia/projects/{ISSUE_ID}.md` until **DB** is healthy or the user **explicitly waives** journal capture. |
