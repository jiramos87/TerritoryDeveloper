...
6b. **Orchestrator stage-complete check** — After flipping task → `Done`, scan the parent stage's task table. If **all** tasks in that stage are now `Done` or `Done (archived)`, emit a visible reminder: _"Stage {STAGE_ID} fully closed — run `project-stage-close` on `{ORCHESTRATOR_PATH}` before starting the next stage."_ Do NOT run `project-stage-close` automatically — surface the reminder only so the user controls the handoff.
