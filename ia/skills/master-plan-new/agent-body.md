...
7. **Persist (DB MCP)** — `master_plan_insert({slug, title, preamble})` → seeds row + preamble. Per Stage authored: `stage_insert({slug, stage_id, title, body, objective, exit_criteria})`. `master_plan_change_log_append({slug, kind: "plan_authored", body})` → audit row. No filesystem write.
