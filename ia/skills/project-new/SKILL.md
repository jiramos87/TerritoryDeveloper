...
This skill **does not** call MCP tools itself. In an **Agent** chat with **territory-ia** enabled, follow the **Tool recipe** below **before** editing **`BACKLOG.md`** or creating **`ia/projects/{ISSUE_ID}.md`**, so vocabulary and spec slices load as **thin context** (see **`AGENTS.md`** step 3 and **`ia/rules/mcp-ia-default.md`**).
...
**Related:** Sibling skills — **kickoff** / **implement** / **close** / [**`project-implementation-validation`**](../project-implementation-validation/SKILL.md). Open **MCP** / **Skills** / **project-spec** hygiene rows — [`BACKLOG.md`](../../../BACKLOG.md). Shipped skill trace — [`BACKLOG-ARCHIVE.md`](../../../BACKLOG-ARCHIVE.md). **Conventions:** [`ia/skills/README.md`](../README.md).
...
3. **`router_for_task`** — One call per **1–3** inferred domains. The `domain` argument must match a **Task domain** (or geography quick-reference) substring from **`ia/rules/agent-router.md`** — see [`ia/skills/README.md`](../README.md) (**`router_for_task`** lesson). Ad-hoc phrases often return **`no_matching_domain`**.

4. **`spec_section`** — For each routed spec, fetch **only** the sections the prompt implies; set **`max_chars`**. **Do not** read entire `ia/specs/*.md` files unless **`spec_outline`** forces it. If the work mentions **Editor → Reports** or **`tools/reports/`**, include **unity-development-context** **§10**.
...
4. **Backlog row** — Include **Type**, **Files**, **Notes**, **Spec:** **`ia/projects/{ISSUE_ID}.md`**, **Depends on** / **Acceptance** as appropriate. Every **`BUG-`/`FEAT-`/`TECH-`-** id you cite must exist in **`BACKLOG.md`** (or be reserved in the **same** edit batch you add it).

5. **Project spec** — Copy [`ia/templates/project-spec-template.md`](../../templates/project-spec-template.md) to **`ia/projects/{ISSUE_ID}.md`**. Fill header (**Issue** link to **`BACKLOG.md`**), **Summary**, **Goals**, stub **Implementation Plan**, **Open Questions** per [`ia/projects/PROJECT-SPEC-STRUCTURE.md`](../../projects/PROJECT-SPEC-STRUCTURE.md) (tooling-only: **None** or point to **Acceptance**).

6. **Validate** — Run **`npm run validate:dead-project-specs`** from the repo root after adding or changing **`Spec:`** paths or links to **`ia/projects/*.md`**.
