...
- **Handoff messages** from Stage-scoped `/closeout` (`stage-closeout-plan` + `plan-applier` Mode stage-closeout) + any future stage-handoff surfaces — message itself caveman-shaped; include explicit "follow `caveman:caveman` skill rules" line forwarded verbatim to next stage's fresh agent.
