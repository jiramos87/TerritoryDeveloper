...
- Issue ids (`BUG-` / `FEAT-` / `TECH-` / `ART-` / `AUDIO-`) appear in `ia/backlog/{id}.yaml` and `ia/backlog-archive/{id}.yaml` (filename stem + `id:` field) and in the generated views `BACKLOG.md` (open) and `BACKLOG-ARCHIVE.md` (closed). They never appear in glossary, reference specs, rules (except this line), skills, `docs/`, or code comments.
