...
- Status enum: `Draft | In Review | In Progress — Stage {N.M} / TECH-XX | Final`.
  ...
  - `Final` — all Stages read `Final`; flipped by `project-stage-close` / `closeout-apply` (R5). `master-plan-extend` demotes back to `In Progress — Stage {N_new}.1 / TECH-XX` when new Stages appended to a Final plan (R6).
...
| R5 | All Stages Final | `project-stage-close` / `closeout-apply` | Plan top `In Progress` → `Final` |
| R6 | New Stages appended to Final plan | `master-plan-extend` | Plan top `Final` → `In Progress — Stage {N_new}.1 / TECH-XX` |

R3 (Stage rollup `In Progress → Final` on last Task archived) is owned by `project-stage-close` — see `project-hierarchy.md`. Phase-flip rules dropped (no Phase level under 2-level hierarchy). Skeleton-flip rule dropped (`master-plan-new` decomposes ALL Stages at author time; no Stage skeletons).
