...
**Documentation hierarchy:** `AGENTS.md` (workflow) → `ia/rules/` (guardrails) → `ia/specs/` (deep reference) → `ia/specs/architecture/layers.md` (dependency map). MCP: prefer `territory-ia` tools over reading whole spec files.

**Verification:** `npm run verify:local` from repo root. See `ia/specs/architecture/interchange.md` (Local verification) and `CLAUDE.md` §5.
