...
> Full dependency map → `ia/specs/architecture/layers.md`.
...
**Desirability** — per-cell attractiveness for zoning + AUTO growth based on terrain context (e.g. proximity to water, forests), computed after geography initialization. See `ia/specs/architecture/data-flows.md` (initialization order, `GeographyManager` desirability pass) when changing how cells become more or less attractive.
