...
### UI Toolkit (iter-43 baseline)

The shipping in-game UI runs on Unity UI Toolkit (UXML + USS + TSS triple) per DEC-A28 strangler. The iter-43 acceptance baseline (recovery plan §15.4) is the current visual contract. Terms below describe the structural pieces; legacy uGUI primitives are listed under §Retired terms.

| Term | Definition | Spec |
|------|-----------|------|
| UXML | XML markup defining a `VisualElement` tree on disk under `Assets/UI/Generated/{slug}.uxml`. One file per panel slug. Hand-authored at iter-43; DB-emitted in Path B. | DEC-A28 / `docs/ui-toolkit-parity-recovery-plan.html` |
| USS | Style sheet (CSS subset) defining per-class visual rules on disk under `Assets/UI/Generated/{slug}.uss`. Co-located with each UXML. Plan-scope rule: literal hex inline, no `var(--ds-*)` cascade. | recovery plan §20 hard rule |
...
| UIDocument | Unity component hosting one UXML asset. Owns a `rootVisualElement` + a `PanelSettings` reference. Scene-attached GameObject (e.g. `notifications-toast-uidoc`, `hud-bar-uidoc`). | recovery plan §15.4 |
| Host MonoBehaviour | Per-panel `*Host.cs` script (e.g. `HudBarHost`, `BudgetPanelHost`, `MapPanelHost`, `StatsPanelHost`, `MainMenuHost`, `NotificationsToastHost`, `HoverInfoHost`) that owns the UIDocument reference, Q-lookups VisualElements by name, wires button callbacks, manages VM state, and registers the panel root with the ModalCoordinator. DEC-A28 invariant I4 — Hosts stay outside the bake pipeline. | DEC-A28 / recovery plan §15.4 |
...
| Runtime VE | Programmatic VisualElement tree built in C# (no UXML asset on disk). Used by `HoverInfoHost` (iter-28) and `MapPanelHost.BuildRuntimePanel` (iter-39+iter-42) when the scene lacks a wired UIDocument or when the surface is too transient to author UXML for. | recovery plan §15.4.3 / TECH-34683 |
| `UxmlBakeHandler` / `UxmlEmissionService` / `TssEmissionService` | DEC-A28 emitter sidecar (`Assets/Scripts/Editor/Bridge/`) — DB rows → UXML+USS pair (`UxmlEmissionService`) and DB tokens → TSS file (`TssEmissionService`). Currently shell stubs; full child-tree walk + per-child USS rule emit lives in Path B (TECH-34678 / TECH-34679). | DEC-A28 / Path B exploration seed |
| iter-43 baseline | The accepted visual contract from the UI Toolkit parity recovery (commits c4ef2a25..d48c5546, plan §15.4). Pixel goldens locked under `tools/visual-baseline/golden/`. Any Path B emitter output that diverges from a locked golden is a regression. | recovery plan §15.4 + `tools/visual-baseline/golden/` |
| Round-3 effort | One of Effort 3..10 in the recovery plan §20–§27 covering Notification toast / Hover-info / Cell click inspect / Mini-map / RCI demand bars / Budget panel / AUTO mode toast / NewGameForm sub-view. Shipped across iter-27..iter-34 + 9 QA rounds. | recovery plan §20..§27 |
| Layer toggle button (Zn / Rd / Fr / Ct) | 28×22px squared `Button` tile inside the MAP panel top-strip overlay, wired to `MiniMapController.ToggleLayer(MiniMapLayer.{Zones, Streets, Forests, Centroid})`. Active state paints `#5b7fa8` blue; inactive paints `#ede4ce` cream. iter-41 final form. | recovery plan §20.4 / iter-41 |
| Notification toast surface | Top-right `notifications-toast` UI Toolkit panel (iter-27..iter-35). 4 type variants (success / warning / error / info) with literal-hex left-accent border. Click-dismiss + 2.5s identical-message dedupe + max-5 stack. `GameNotificationManager.Post*` routes through `NotificationsToastHost.Instance` with silent `Debug.Log` fallback. | recovery plan §20 |
| Hover-info card | Bottom-right `hover-info` runtime VE (iter-28). Per-frame readout of cell coords + type + occupied state + ground height + water surface H when over a water body. Hides on off-grid + over-UI. Anchored above MAP corner per iter-40 lift. | recovery plan §21 |
| Cell click inspect panel | `InfoPanelHost` extended in iter-29 with `GridManager.selectedPoint` polling + `WorldSelectionResolver` field bundle + `DemolishCommand` wired to `GridManager.DemolishAt`. Reuses existing `info-panel` UIDocument + ModalCoordinator slug. | recovery plan §22 |
| MAP panel (runtime VE) | Bottom-right cream square (iter-37+iter-40+iter-42) hosting the procedural mini-map texture from `MiniMapController.MapTexture` + a top-strip layer toggle row. Runtime-spawned by `MapPanelHost.Bootstrap` when no scene UIDoc exists; programmatic VE appended to the notifications-toast UIDocument root. | recovery plan §23 / TECH-34683 |
| Cream theme / dark theme | Two TSS variants under `Assets/UI/Themes/`. iter-43 panels use cream (`#f5e6c8` card / `#b89b5e` border / `#3a2f1c` text / `#5b7fa8` accent). Dark theme retained for legacy panels not yet cutover. | recovery plan §20 hard rule / `Assets/UI/Themes/cream.tss` |
