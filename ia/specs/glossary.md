...
| **Audio** | Bake-to-clip, Blip bootstrap, Blip cooldown, Blip mixer group, Blip patch, Blip patch flat, Blip variant, Patch flatten, patch hash, scene-load suppression |
| **Prefabs** | land/water slopes, sorting formula, type offsets |
| **Documentation** | reference spec, project spec, orchestrator document, project hierarchy, rollout tracker, rollout lifecycle, alignment gate, skill iteration log, per-skill changelog, interchange JSON, geography_init_params, scenario_descriptor_v1, City metrics history, Agent test mode batch, IDE agent bridge |
