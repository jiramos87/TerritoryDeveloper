...
| Feature flag (stage-scoped) | Boolean gate stored in `ia_feature_flags` (slug PK, enabled, default_value, owner, optional `stage_id` FK). Runtime hydrated into `FeatureFlags` static class via `tools/interchange/feature-flags-snapshot.json` at boot (Awake). Global flag: `stage_id IS NULL`. Stage-scoped: `stage_id` points to one ia_stages row. Bridge kind `flag_flip` invalidates cache + re-hydrates without Play Mode restart. | [interchange.md §Interchange artifacts registry](architecture/interchange.md), [FeatureFlags.cs](../../Assets/Scripts/Core/Core/FeatureFlags.cs) |
...
| rollback_token | UUID returned by transactional bridge composites (e.g. `wire_asset_from_catalog`) keying an in-memory `CellSubtreeSnapshot` entry. Eager eviction on commit OR rollback completion; stale-token reuse → `token_unknown` sentinel. | [scene-contract](../../docs/asset-pipeline-scene-contract.md), [arch §DEC-A48](../../docs/asset-pipeline-architecture.md) |
| scene contract | IA doc enumerating the canonical Unity scene path templates the asset-pipeline bridge composite resolves against (toolbar buttons, panel tiers, world cells, bootstrap UIManager). Pre-mutation `unknown_scene_path` rejection contract for offered paths failing template match. | [scene-contract](../../docs/asset-pipeline-scene-contract.md) |
| wire_asset_from_catalog | Bridge composite kind wiring a published catalog `entity_id` onto the active scene under one canonical mount. Returns `{ok, mutations[], rollback_token}` envelope; supports `dry_run=true` preflight; transactional snapshot → mutate → verify → commit/rollback. | [scene-contract](../../docs/asset-pipeline-scene-contract.md), [arch §DEC-A4](../../docs/asset-pipeline-architecture.md), [arch §DEC-A48](../../docs/asset-pipeline-architecture.md) |
| asset-registry | DB-wins authority tier for UI assets (panels, buttons, tokens, archetypes). `catalog_entity` rows with `kind IN ('panel','button','token','archetype')` + `ds_tokens` jsonb + `motion` jsonb columns. Exposed via `asset_registry` view. Source of truth for bake → IR pipeline per asset-pipeline-standard §Authority (DEC-A25). | [asset-pipeline-standard](architecture/asset-pipeline-standard.md) |
| asset-pipeline-standard | Canonical spec governing authoring, baking, validating, and rendering game UI assets end-to-end. Sections: §Mission, §Roles, §Authority, §Validator, §Checklist, §Playbook. Active: DEC-A25 `asset-pipeline-standard-v1`. | [asset-pipeline-standard](architecture/asset-pipeline-standard.md) |
| asset family | One of 10 named groupings of Unity project assets (e.g. `ui-panel`, `terrain-sprite`, `audio-sfx`). Each family maps to a canonical path prefix under `Assets/` and governs where new files of that kind are placed. | [asset-pipeline-standard §Canonical asset paths](architecture/asset-pipeline-standard.md) |
| canonical asset path | The authoritative Unity project file path for an asset, determined by its asset family prefix + kebab-case file stem. Files outside their canonical path fail `validate:asset-tree-canonical`. | [asset-pipeline-standard §Canonical asset paths](architecture/asset-pipeline-standard.md) |
| kebab-case asset name | File stem format required for all Unity project assets: `^[a-z0-9]+(-[a-z0-9]+)*$`. Lowercase alphanumeric segments separated by hyphens; no underscores, spaces, or uppercase. Enforced by `validate:asset-tree-canonical`. | [asset-pipeline-standard §Canonical asset paths](architecture/asset-pipeline-standard.md) |
| archetype_version | Versioned archetype YAML row consumed by sprite-gen `render_run` invocations. Bumps on archetype spec edit; render_run pins one version for reproducibility. | [catalog-architecture](catalog-architecture.md) §Archetype authoring |
| catalog_entity | Spine row representing one catalog asset (panel, button, sprite, archetype, token, audio, pool). Slug MUST follow `{purpose}-{kind}` convention per `ia/rules/catalog-naming.md`. All cross-kind operations (search, retire, audit) live on the spine. | [catalog-architecture](catalog-architecture.md) §2.1, [catalog-naming](../rules/catalog-naming.md) |
| blob_resolver | Mediator translating `gen://{archetype_slug}/v{NN}` URIs to physical paths under `BLOB_ROOT`. Single env-var swap point (DEC-A25) for local + future hosted blob stores. | [catalog-architecture](catalog-architecture.md) §Blob resolver protocol, [arch §DEC-A25](../../docs/asset-pipeline-architecture.md) |
| capability | Proxy-level access gate keyed by user role. Routes declare `routeMeta: {METHOD: {requires}}`; `proxy.ts` checks the role's capability set + returns DEC-A48 forbidden envelope on miss. | [catalog-architecture](catalog-architecture.md) §Capability matrix, [arch §DEC-A33](../../docs/asset-pipeline-architecture.md) |
| entity_version | Per-edit version row for a catalog entity. Append-only history surface; latest published version is what snapshots export. | [catalog-architecture](catalog-architecture.md) §Entity version lifecycle |
| panel_child | Per-row composition entry on a `panel_detail` row — child slot id + child entity ref + ordinal. Drives panel composition rendering. | [catalog-architecture](catalog-architecture.md) §Spine + Detail pattern |
| pool_member | Per-row entry on a spawn-pool — pool id + member entity ref + weight + optional condition predicate. Drives pool member resolution at spawn time. | [catalog-architecture](catalog-architecture.md) §Spine + Detail pattern |
| publish_lint_rule | Pre-snapshot guard run by the publish pipeline. Two layers: L1 (schema/shape — required fields, ref integrity) + L2 (semantic — capability coverage, blob resolvability). Failure blocks publish with structured diagnostic envelope. | [catalog-architecture](catalog-architecture.md) §Publish lint layers |
| render_run | One sprite-gen invocation row — pinned `archetype_version` + seed + variant outputs (`gen://` blob refs). `/catalog/render-runs` browser surface. | [catalog-architecture](catalog-architecture.md) §Render run lifecycle |
| snapshot | Published catalog export consumed by Unity loader. Versioned bundle of entity rows + detail tables + blob manifest at one publish moment. `/catalog/snapshots` browser surface. | [catalog-architecture](catalog-architecture.md) §Snapshot export contract |
...
### UI Toolkit (current UI baseline)

The shipping in-game UI runs on Unity UI Toolkit (UXML + USS + TSS triple) per DEC-A28 strangler. The current pixel-locked goldens under `tools/visual-baseline/golden/` are the visual contract. Terms below describe the structural pieces; legacy uGUI primitives are listed under §Retired terms.

| Term | Definition | Spec |
|------|-----------|------|
| UXML | XML markup defining a `VisualElement` tree on disk under `Assets/UI/Generated/{slug}.uxml`. One file per panel slug. Hand-authored under the current UI baseline; DB-emitted in Path B. | DEC-A28 / `docs/ui-toolkit-parity-recovery-plan.html` |
| USS | Style sheet (CSS subset) defining per-class visual rules on disk under `Assets/UI/Generated/{slug}.uss`. Co-located with each UXML. Per-panel `.uss` files use literal hex inline (no `var(--ds-*)` cascade). | `Assets/UI/Generated/` |
| TSS | Theme style sheet under `Assets/UI/Themes/{theme}.tss` (cream.tss / dark.tss) defining `:root { --ds-color-*: hex }` token variables. Currently hand-authored; Path B TECH-34681 makes it DB-canonical. | DEC-A28 §I1 |
| VisualElement | UI Toolkit runtime node type. Root of any UXML tree, parent of all UI elements (`Button`, `Label`, `Slider`, `Toggle`, `DropdownField`, `IntegerField`, `TextField`, `ScrollView`). Mounted under a `UIDocument` component. | Unity UI Toolkit |
| UIDocument | Unity component hosting one UXML asset. Owns a `rootVisualElement` + a `PanelSettings` reference. Scene-attached GameObject (e.g. `notifications-toast-uidoc`, `hud-bar-uidoc`). | Unity UI Toolkit |
| Host MonoBehaviour | Per-panel `*Host.cs` script (e.g. `HudBarHost`, `BudgetPanelHost`, `MapPanelHost`, `StatsPanelHost`, `MainMenuHost`, `NotificationsToastHost`, `HoverInfoHost`) that owns the UIDocument reference, Q-lookups VisualElements by name, wires button callbacks, manages VM state, and registers the panel root with the ModalCoordinator. DEC-A28 invariant I4 — Hosts stay outside the bake pipeline. | DEC-A28 / `Assets/Scripts/UI/Hosts/` |
| ModalCoordinator | MonoBehaviour facade managing modal panel visibility. `RegisterMigratedPanel(slug, root)` registers a UI Toolkit panel; `Show(slug)` + `HideMigrated(slug)` toggle `style.display = Flex/None` on the registered root + inner slug-named VisualElement. Drives exclusive-group close behaviour. | `Assets/Scripts/UI/Modals/ModalCoordinator.cs` |
| Runtime VE | Programmatic VisualElement tree built in C# (no UXML asset on disk). Used by `HoverInfoHost` and `MapPanelHost.BuildRuntimePanel` when the scene lacks a wired UIDocument or when the surface is too transient to author UXML for. | TECH-34683 / `Assets/Scripts/UI/Hosts/` |
| `UxmlBakeHandler` / `UxmlEmissionService` / `TssEmissionService` | DEC-A28 emitter sidecar (`Assets/Scripts/Editor/Bridge/`) — DB rows → UXML+USS pair (`UxmlEmissionService`) and DB tokens → TSS file (`TssEmissionService`). Currently shell stubs; full child-tree walk + per-child USS rule emit is the Path B scope (TECH-34678 / TECH-34679). | DEC-A28 / `docs/explorations/ui-toolkit-emitter-parity-and-db-reverse-capture.md` |
| Current UI baseline | The accepted visual contract for the shipping in-game UI. Pixel goldens locked under `tools/visual-baseline/golden/`; any Path B emitter output that diverges from a locked golden is a regression. The recovery effort archive lives in `docs/ui-toolkit-parity-recovery-plan.html` as historical authoring record only. | `tools/visual-baseline/golden/` |

| Layer toggle button (Zn / Rd / Fr / Ct) | 28×22px squared `Button` tile inside the MAP panel top-strip overlay, wired to `MiniMapController.ToggleLayer(MiniMapLayer.{Zones, Streets, Forests, Centroid})`. Active state paints `#5b7fa8` blue; inactive paints `#ede4ce` cream. | `tools/visual-baseline/golden/` |
| Notification toast surface | Top-right `notifications-toast` UI Toolkit panel. 4 type variants (success / warning / error / info) with literal-hex left-accent border. Click-dismiss + 2.5s identical-message dedupe + max-5 stack. `GameNotificationManager.Post*` routes through `NotificationsToastHost.Instance` with silent `Debug.Log` fallback. | `Assets/Scripts/UI/Hosts/NotificationsToastHost.cs` |
| Hover-info card | Bottom-right `hover-info` runtime VE. Per-frame readout of cell coords + type + occupied state + ground height + water surface H when over a water body. Hides on off-grid + over-UI. Anchored above MAP corner. | `Assets/Scripts/UI/Hosts/HoverInfoHost.cs` |
| Cell click inspect panel | `InfoPanelHost` with `GridManager.selectedPoint` polling + `WorldSelectionResolver` field bundle + `DemolishCommand` wired to `GridManager.DemolishAt`. Reuses existing `info-panel` UIDocument + ModalCoordinator slug. | `Assets/Scripts/UI/Hosts/InfoPanelHost.cs` |
| MAP panel (runtime VE) | Bottom-right cream square hosting the procedural mini-map texture from `MiniMapController.MapTexture` + a top-strip layer toggle row. Runtime-spawned by `MapPanelHost.Bootstrap` when no scene UIDoc exists; programmatic VE appended to the notifications-toast UIDocument root. | `Assets/Scripts/UI/Hosts/MapPanelHost.cs` / TECH-34683 |
| Cream theme / dark theme | Two TSS variants under `Assets/UI/Themes/`. Current UI baseline panels use cream (`#f5e6c8` card / `#b89b5e` border / `#3a2f1c` text / `#5b7fa8` accent). Dark theme retained for legacy panels not yet cutover. Per-panel `.uss` files use literal hex inline (no `var(--ds-*)` cascade). | `Assets/UI/Themes/cream.tss` |
...
| service registry | `Domains/_Registry/ServiceRegistry` MonoBehaviour facade locator. Scene-hosted; producers call `Register<T>` in Awake, consumers call `Resolve<T>` in Start. One instance per scene. Invariant #12. | `docs/post-atomization-architecture.md §Service Registry` |
...
| `UiBindRegistry` (reactive bind table) | Native UI Toolkit `INotifyValueChanged` / `ChangeEvent<T>` bindings | Stage 6.0 (`ui-toolkit-migration`) — TECH-32927 quarantine. `[Obsolete]` set on class + members; deletion deferred to uGUI purge plan. |
| `UiTheme` ScriptableObject | USS `var(--ds-*)` token consumption via UI Toolkit TSS | Stage 6.0 (`ui-toolkit-migration`) — TECH-32928 quarantine. `[Obsolete]` set; runtime panels consume TSS only. Deletion deferred to uGUI purge plan. |
| `ThemedPrimitiveBase` + subclasses (`ThemedLabel`, `ThemedButton`, `ThemedFrame`, `ThemedList`, `ThemedTabBar`, `ThemedIcon`, `ThemedDivider`, `ThemedBadge`, `ThemedPanel`, `ThemedToggle`, `ThemedSlider`, `ThemedTooltip`, `ThemedSectionHeader`, `ThemedIlluminationLayer`, `ThemedOverlayToggleRow`) | USS classes / `[UxmlElement]` custom-control ports / UI Toolkit built-in elements | Stage 6.0 (`ui-toolkit-migration`) — TECH-32929 quarantine. All wrappers `[Obsolete]`; complex primitives retain wrapper alongside UxmlElement port. Deletion deferred to uGUI purge plan. |
| `IThemed` interface | No direct replacement (absorbed into UI Toolkit VisualElement composition) | Stage 6.0 (`ui-toolkit-migration`) — TECH-32929 quarantine. Interface `[Obsolete]`; deletion deferred to uGUI purge plan. |
