...
| asset-registry | DB-wins authority tier for UI assets (panels, buttons, tokens, archetypes). `catalog_entity` rows with `kind IN ('panel','button','token','archetype')` + `ds_tokens` jsonb + `motion` jsonb columns. Exposed via `asset_registry` view. Source of truth for bake → IR pipeline per asset-pipeline-standard §Authority (DEC-A25). | [asset-pipeline-standard](asset-pipeline-standard.md) |
| asset-pipeline-standard | Canonical spec governing authoring, baking, validating, and rendering game UI assets end-to-end. Sections: §Mission, §Roles, §Authority, §Validator, §Checklist, §Playbook. Active: DEC-A25 `asset-pipeline-standard-v1`. | [asset-pipeline-standard](asset-pipeline-standard.md) |
| asset family | One of 10 named groupings of Unity project assets (e.g. `ui-panel`, `terrain-sprite`, `audio-sfx`). Each family maps to a canonical path prefix under `Assets/` and governs where new files of that kind are placed. | [asset-pipeline-standard §Canonical asset paths](asset-pipeline-standard.md) |
| canonical asset path | The authoritative Unity project file path for an asset, determined by its asset family prefix + kebab-case file stem. Files outside their canonical path fail `validate:asset-tree-canonical`. | [asset-pipeline-standard §Canonical asset paths](asset-pipeline-standard.md) |
| kebab-case asset name | File stem format required for all Unity project assets: `^[a-z0-9]+(-[a-z0-9]+)*$`. Lowercase alphanumeric segments separated by hyphens; no underscores, spaces, or uppercase. Enforced by `validate:asset-tree-canonical`. | [asset-pipeline-standard §Canonical asset paths](asset-pipeline-standard.md) |
