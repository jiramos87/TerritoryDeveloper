...
> **Agent tooling:** The **territory-ia** MCP tool **`isometric_world_to_grid`** (implementation in **`tools/compute-lib`**) applies the **inverse** formulas above to **planar** world coordinates only. It does **not** implement height-aware picking below.
...
- **Cell child cleanup (overlays):** When clearing **Cell** children to place **utility buildings**, **RCI** **buildings**, or **undeveloped light zoning** overlays, do **not** destroy **brown cliff** stack instances or **water–water cascade** cliff instances parented under that **cell**; use the same prefab-instance identification as when removing **cliffs** for terrain refresh. Preserve **land**/**water slope** children on those paths as well. Bulldoze flows that run full terrain restore still rebuild **cliffs** via the normal **cliff** placement pass (which clears existing stack children first). Rationale: **`RestoreTerrainForCell`** does not rebuild **cliffs** on **cells** already marked **building**-occupied, so stripping stacks during overlay placement would leave **map border** gaps toward **off-grid void**.
