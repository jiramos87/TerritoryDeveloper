---
purpose: "TECH-860 — Fixtures for `related` existence check."
audience: both
loaded_by: ondemand
slices_via: none
...
task_key: "T5.2"
---
# TECH-860 — Fixtures for `related` existence check

> **Issue:** [TECH-860](../../BACKLOG.md)
> **Status:** Draft
> **Created:** 2026-04-24
...
Add to `tools/scripts/test-fixtures/` — `related-exists-pass/` (two records, one refers to the other), `related-exists-fail/` (record refers to nonexistent id). Extend fixture harness to assert pass/fail outcomes + expected error text. Scoped to Stage 5 of `ia/projects/backlog-yaml-mcp-alignment-master-plan.md` — validator extensions (IP8). Tooling-only, no runtime C# touches.

## 2. Goals and Non-Goals

### 2.1 Goals

1. fixture pass/fail pair for related existence check.
...
|---|------|-------|---------------------|
| 1 | Developer | As a developer, I want fixture pass/fail pair for related existence check so that backlog yaml drift surfaces before merge. | `npm run validate:backlog-yaml` exits non-zero on failing fixture. |
...
Add to `tools/scripts/test-fixtures/` — `related-exists-pass/` (two records, one refers to the other), `related-exists-fail/` (record refers to nonexistent id). Extend fixture harness to assert pass/fail outcomes + expected error text.
...
- [ ] fixture pass/fail pair for related existence check.
