...
- Risk: C# default struct zeroing sets `PoolStatus` to `0` — must match first enum value `Healthy` (TECH-331). Mitigation: confirm `Healthy = 0` explicitly in TECH-331 or document implicit; add XML on struct that default ctor is valid seed.
- Risk: `float` fields allow NaN — EMA math in Stage 1.2 can propagate. Mitigation: Stage 1.2 owns guards; this task only documents “initialized to zero” in XML.
- Ambiguity: `struct` vs `readonly struct`. Resolution: plain `struct` per Decision Log unless Burst annotation required later.
- Invariant touch: blittable struct — no `string` or reference fields.

### §Examples

| Case | Expected | Notes |
|------|----------|-------|
| `default(PoolState)` | `net=0`, `ema=0`, `status=Healthy`, counters `0` | Unity + C# default rules |
| Dictionary value mutation | Stage 1.2 mutates fields in place | No replacement of whole struct without care |

### §Test Blueprint

| test_name | inputs | expected | harness |
|-----------|--------|----------|---------|
| compile_pool_state | TECH-331 merged | `npm run unity:compile-check` exit 0 | unity |
| validate_ia | repo | `npm run validate:all` exit 0 | node |

### §Acceptance

- [ ] `PoolState.cs` with five public fields and XML docs.
- [ ] No managed reference fields inside struct.
- [ ] `npm run validate:all` green.

### §Findings

- Hysteresis counter semantics (when to increment) owned by Stage 1.2 `UtilityPoolService` — not asserted in this spec.
