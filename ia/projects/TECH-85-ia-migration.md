...
> **Status:** Final (Stage 1 + Stage 2 + Stage 3 + Stage 4 closed 2026-04-10; Stage 5 ready for fresh agent)
> **Created:** 2026-04-10
...
**Caveman rules are mandatory across all subagents, skills, and handoff prompts.** Every subagent body, every `ia/skills/{name}/SKILL.md` recipe authored or updated under TECH-85, and every handoff message emitted by `project-stage-close` (or any future stage-handoff surface) must instruct the executing agent to follow `caveman:caveman` skill rules for all responses, mirroring the global default in `~/.claude/CLAUDE.md`. The directive does **not** apply to code, commits, security/auth content, or verbatim error messages — those follow the standard caveman exceptions. Rationale: subagents start in a fresh context window and do **not** inherit the parent session's caveman SessionStart hook, so without an explicit directive in the subagent file they fall back to verbose default phrasing and burn the parent's token budget when their report flows back. Same logic applies to handoff prompts pasted into a fresh stage agent.
...
8. Emit a **handoff message** the human can paste verbatim to the next stage's fresh agent (issue id, stage id, branch state, verification summary, pointer to this spec). **The handoff message itself must be written in caveman style** and must include an explicit line instructing the next agent to follow `caveman:caveman` skill rules for the entire stage. Rationale: the next stage runs in a fresh context with no inherited SessionStart hook state, so the directive has to live inside the pasted prompt or it will not be applied.
...
| 2026-04-10 | **Caveman rules are mandatory across all subagents, skills, and handoff prompts authored under TECH-85.** Every `.claude/agents/*.md` body, every `ia/skills/{name}/SKILL.md` recipe touched by Stage 4 (and `project-stage-close/SKILL.md`), every slash command body in `.claude/commands/`, and every handoff message emitted by `project-stage-close` must instruct the executing agent to follow `caveman:caveman` skill rules for all responses, with the standard caveman exceptions (code, commits, security/auth content, verbatim error/tool output). Spec sections updated: §5.2 (Subagents), §5.3 step 8 (handoff), Phase 4.1 / 4.3, §8 (acceptance). | Subagents and freshly-paste-handed stage agents start in a clean context window and **do not inherit** the parent session's caveman SessionStart hook (`~/.claude/CLAUDE.md` says so explicitly: "the caveman plugin's SessionStart hook auto-activates this in main sessions but does not propagate to subagents"). Without an explicit directive baked into each subagent / skill / handoff prompt, every subagent reverts to verbose default phrasing and burns the parent context budget when its report flows back. Belt-and-suspenders: directive lives in (a) the subagent body, (b) the skill recipe preamble, and (c) the slash-command prompt — so removing it from one surface still leaves the other two enforcing it. | Rely on the global `~/.claude/CLAUDE.md` directive alone (rejected: confirmed not to propagate to subagents); add caveman only at the slash-command surface (rejected: subagents invoked outside slash commands miss it); add caveman only in subagent files (rejected: skills invoked directly without a subagent miss it); make caveman conditional on issue type (rejected: complexity for marginal benefit) |
...
| 2026-04-10 | **Subagent frontmatter convention: Claude Code native shape (`name`, `description`, `tools`, `model`) rather than the four-field IA frontmatter.** `.claude/agents/*.md` lives outside `ia/` and is consumed by the Claude Code host directly, not by the territory-ia MCP server. The four-field IA schema (`purpose`, `audience`, `loaded_by`, `slices_via`) does not apply; `validate:frontmatter` does not scan `.claude/`. Each subagent body still carries an explicit `caveman:caveman` directive paragraph above the SKILL.md reference, per §5.2 + §6 caveman decision row. | Mixing the IA schema into native Claude Code subagents would either confuse the host's frontmatter parser or force a custom-key extension. The native shape is what the Claude Code host actually reads (`tools` enforces the per-tool allowlist for the subagent's isolated context; `model` chooses Opus vs Sonnet per §6 model-split row). | Use the four-field IA frontmatter on subagents (rejected: not what Claude Code parses); skip frontmatter entirely (rejected: subagent loses tools / model assignment); store subagents under `ia/` with IA frontmatter (rejected: `.claude/agents/` is the canonical Claude Code location and the host only scans there) |
| 2026-04-10 | **`tools` field on each subagent enumerates the explicit MCP tool names instead of using the `mcp__territory-ia__*` wildcard.** The session-level `permissions.allow` wildcard remains canonical (TECH-85 §6 row from 2026-04-10, §9 issue #4) and continues to suppress per-call approval prompts at the **parent** session boundary. Subagent `tools` fields, however, declare the **subset** the subagent is allowed to call from its isolated context. Each subagent body lists exactly the MCP tools its skill recipe needs (e.g. `verifier` only gets `unity_bridge_command` / `unity_bridge_get` / `unity_compile` / preflight / list_rules; `closeout` only gets `backlog_*` / `spec_*` / `glossary_*` / `project_spec_*` / `list_*`). | The Claude Code host's subagent `tools` field is allow-list semantics, not wildcard semantics; using the wildcard at the subagent layer would over-broaden the subagent's reach beyond what its recipe actually exercises. Per-subagent enumeration scopes side effects (e.g. `verifier` cannot persist a journal entry; `spec-kickoff` cannot launch the Unity bridge). The session-level wildcard still carries the per-call ergonomics. | Wildcard at the subagent layer (rejected: over-broadens reach + masks side-effect surface); empty `tools` (rejected: subagent inherits parent toolset which is too permissive); per-subagent `permissions.deny` (rejected: subagent files have no permission block) |
| 2026-04-10 | **Output style format: fenced JSON header + caveman markdown summary, in that order.** `.claude/output-styles/verification-report.md` and `closeout-digest.md` both encode the structure as: (1) one fenced ` ```json ` block with the canonical fields (`{validate_all, compile, batch, bridge}` for verifier; `{issue_id, spec_path, lessons_migrated, journal_persist, validate_*, confirmation_gate, spec_deleted, backlog_row_removed, archive_appended, id_purged_from, validate_dead_specs_post}` for closeout), (2) caveman markdown summary in the same order as the JSON. The JSON header is **exempt** from caveman compression so downstream tooling can parse it; the summary applies caveman to the prose. | Two consumers want different things: machine-readable contract for automation (CI parsing, dashboards, future Stage 5 MCP code-intelligence ingestion) and human-readable signal for the developer reading the chat. Embedding both in one block keeps the agent surface unified — there is one Verification block, not two. The JSON-first ordering means the contract is at the top of the message where parsers can grep for the fence, and the summary follows for the human to skim. | YAML header instead of JSON (rejected: every downstream tool already speaks JSON; YAML adds parser surface for no benefit); JSON-only no summary (rejected: kills the human signal); summary-only no JSON (rejected: loses the machine-readable contract); two separate blocks (rejected: downstream tooling has to glue them back together) |
| 2026-04-10 | **Slash command bodies use the prompt-template pattern: a markdown body that the parent agent reads and forwards to the subagent via the Agent tool.** The slash command file does **not** use a special "dispatch directive" — Claude Code's slash command shape is "expand the body into the prompt that the parent agent receives". The body therefore carries instructions like "Use the **`spec-kickoff`** subagent... Forward the following prompt to the subagent via the Agent tool with `subagent_type: \"spec-kickoff\"`". Each forwarded prompt re-asserts the caveman directive (belt-and-suspenders). | Confirmed empirically during Stage 4 smoke testing: the in-session `Agent` tool only sees built-in subagent types (`general-purpose`, `Explore`, `Plan`, `claude-code-guide`, `statusline-setup`); user-defined subagents under `.claude/agents/` are reachable only through the slash-command path that the Claude Code host parses and dispatches outside the in-session tool surface. The slash command body becomes the prompt that the host hands to the subagent when the user types `/verify`, `/kickoff TECH-85`, etc. The instructions in the body then guide the subagent's behavior in its fresh context. | Make the slash command body a literal `<task subagent_type="..."/>` tag (rejected: not the Claude Code shape); store dispatch metadata in frontmatter (rejected: same — Claude Code reads body, not custom frontmatter keys); have each command file invoke the underlying skill directly without subagent indirection (rejected: loses the isolated context window benefit that motivated the subagent split in the first place) |
| 2026-04-10 | **Claude Code host slash-command smoke is "manual / dev machine", not in-session agent smoke.** The Stage 4 verification step "invoke each slash command on a known issue (e.g., `/verify`, `/kickoff TECH-85`)" cannot be performed by the in-session agent itself — user-defined subagents are not reachable via the in-session `Agent` tool surface. Stage 4 close did the filesystem-level smoke instead (frontmatter parses, body carries caveman directive, expected subagent referenced from each command body, no `Stage 1 stub` / `Stage 4 stub` / `not yet wired` language remaining), and explicitly defers the interactive Claude Code UI smoke to the user (same pattern as Stage 1 hook smoke and Stage 2 Cursor `alwaysApply` smoke). | The acceptance criterion is "subagent boots in isolated context; output style applied for `/verify`; caveman directive observed in subagent responses". The first and third are testable at the filesystem layer (file exists, frontmatter shape valid, body carries directive). The second (output style binding) is also a host-side concern that filesystem inspection can confirm (`.claude/output-styles/verification-report.md` exists with the expected JSON-then-summary shape). The remaining "subagent runs in isolation" check requires the human to type the slash command in Claude Code itself; the agent that executes Stage 4 cannot dispatch into its own host UI. | Spawn a sub-process to launch a fresh Claude Code session and pipe a slash command into it (rejected: brittle, host-version-dependent, leaks state across sessions); skip the smoke entirely (rejected: §8 acceptance is explicit); claim the in-session Agent tool will dispatch user-defined subagents (rejected: empirically false — confirmed `Agent type 'verifier' not found. Available agents: general-purpose, statusline-setup, Explore, Plan, claude-code-guide`) |
| 2026-04-10 | **Stage 4 close.** All Phase 4.1 / 4.1b / 4.2 / 4.3 / 4.4 checkboxes ticked. 5 subagents authored under `.claude/agents/` (`spec-kickoff`, `spec-implementer`, `verifier`, `test-mode-loop`, `closeout`) with native Claude Code frontmatter, model split (Opus orchestrators / Sonnet executors), per-subagent `tools` allow-list, and explicit `caveman:caveman` directive paragraph in each body. 6 skill recipes carry the caveman preamble (`project-spec-kickoff`, `project-spec-implement`, `project-implementation-validation`, `agent-test-mode-verify`, `project-spec-close`, `project-stage-close`); the `project-stage-close` handoff template now forwards the caveman directive verbatim into the next stage's pasted prompt. 2 output styles authored under `.claude/output-styles/` (`verification-report`, `closeout-digest`) — JSON header exempt from caveman, markdown summary applies caveman. 5 slash commands rewritten under `.claude/commands/` with subagent dispatch + caveman re-assertion in the forwarded prompt (belt-and-suspenders). `CLAUDE.md` updated: §3 "Key files" table now lists `.claude/agents/`, `.claude/output-styles/`, the 5 dispatcher commands, and the 6 skill recipes — no more "Stage 4 stub" language. `npm run validate:all` exit 0 (123/123 tests; build-registry test fixture updated to expect 25 IA entries / 13 rules after `ia/rules/agent-output-caveman.md` joined the tree); `npm run validate:frontmatter` OK (74 files); `npm run verify:local` end-to-end exit 0 (validate:all + unity:compile-check + db:migrate + db:bridge-preflight + macOS Editor save/quit + db:bridge-playmode-smoke all green, real Unity Play Mode bridge response received); `project-stage-close` step 7 sanity check OK (`defaultMode == acceptEdits`, `mcp__territory-ia__*` in `allow`). Caveman coverage gate: 16 paths (5 agents + 5 commands + 6 skills) all carry the directive. | Stage 4 final verification per §7 Phase 4.4 + agent-led verification policy + §8 caveman-coverage acceptance row. | — |
...
**Caveman directive (applies to every subagent file below):** each subagent body must contain an explicit instruction to follow `caveman:caveman` skill rules for all responses, with the standard exceptions (code, commits, security/auth content, verbatim error messages). Place the directive near the top of the body, above the SKILL.md reference, so it survives any future densification pass. Rationale captured in §5.2 and §6 Decision Log.

- [x] `.claude/agents/spec-kickoff.md` — frontmatter + body referencing `ia/skills/project-spec-kickoff/SKILL.md`; tools include Read, Grep, Glob, Edit, mcp__territory-ia__*; **Opus**; caveman directive present.
- [x] `.claude/agents/spec-implementer.md` — references `ia/skills/project-spec-implement/SKILL.md`; full editing toolset; **Opus**; caveman directive present.
- [x] `.claude/agents/verifier.md` — references `docs/agent-led-verification-policy.md`; tools limited to Bash + MCP bridge tools; **Sonnet**; instructed to emit JSON for the `verification-report` output style; caveman directive present (the structured JSON header is exempt — caveman applies to the human-readable summary that follows it).
- [x] `.claude/agents/test-mode-loop.md` — references `ia/skills/agent-test-mode-verify/SKILL.md`; **Sonnet**; caveman directive present.
- [x] `.claude/agents/closeout.md` — references `ia/skills/project-spec-close/SKILL.md` (umbrella close, **not** stage close); **Opus**; **requires explicit user confirmation before destructive operations** (per Q6 resolution); caveman directive present (confirmation prompts and destructive-op warnings stay in normal English so the human is not asked to disambiguate fragments).
- [x] **Caveman retrofit on the 5 corresponding skill recipes.** Each `ia/skills/{name}/SKILL.md` consumed by the subagents above (`project-spec-kickoff`, `project-spec-implement`, `project-implementation-validation`, `agent-test-mode-verify`, `project-spec-close`) gains a one-paragraph "Output style" preamble pointing at `caveman:caveman` skill rules with the standard exceptions, so direct skill invocations (not via subagent) inherit the same default. Apply the same preamble to `project-stage-close/SKILL.md` so the inline stage-close path also produces caveman handoff messages. *(Also extended `project-stage-close/SKILL.md` handoff template to forward the caveman directive verbatim into the Stage N+1 paste-ready prompt.)*

#### Phase 4.2 — Output styles

- [x] `.claude/output-styles/verification-report.md` — structure spec for the Verification block (JSON header + markdown summary).
- [x] `.claude/output-styles/closeout-digest.md` — structure spec for the closeout report.

#### Phase 4.3 — Real slash commands replace Stage 1 stubs

Each command body must dispatch the subagent **and** repeat the caveman directive in the prompt it forwards (belt-and-suspenders: even if a future edit accidentally strips the directive from the subagent file, the slash command surface still reasserts it).

- [x] Replace stubs in `.claude/commands/`:
  - `kickoff.md` → invokes `spec-kickoff` subagent with `ia/projects/{ID}*.md`; prompt asserts caveman rules
  - `implement.md` → invokes `spec-implementer` with same path resolution; prompt asserts caveman rules
  - `verify.md` → invokes `verifier`; prompt asserts caveman rules (JSON header exempt)
  - `testmode.md` → invokes `test-mode-loop`; prompt asserts caveman rules
  - `closeout.md` → invokes `closeout` (umbrella close); prompt asserts caveman rules (confirmation prompts exempt)

#### Phase 4.4 — `CLAUDE.md` update + Stage 4 verification + close

- [x] Update `CLAUDE.md` with the 5 slash commands and 5 subagents (no more Stage 1 stub language).
- [x] Verification: invoke each slash command on a known issue (e.g., `/verify`, `/kickoff TECH-85`); confirm subagent runs in isolation; confirm output style applied for `/verify`.
- [x] Run `project-stage-close` — handoff to Stage 5 agent.
...
| Stage 4 caveman directive coverage on subagents/skills/commands | Bash | `grep -l 'caveman:caveman' .claude/agents/*.md .claude/commands/*.md ia/skills/{project-spec-kickoff,project-spec-implement,project-implementation-validation,agent-test-mode-verify,project-spec-close,project-stage-close}/SKILL.md` returns all 16 paths (5 agents + 5 commands + 6 skills) | per §6 caveman decision row |
...
- [ ] **Caveman directive present on every Stage 4 surface:** `grep -l 'caveman:caveman' .claude/agents/*.md` lists all 5 subagents; the 5 corresponding skill recipes (`project-spec-kickoff`, `project-spec-implement`, `project-implementation-validation`, `agent-test-mode-verify`, `project-spec-close`) and `project-stage-close/SKILL.md` each carry a caveman preamble; every slash command body in `.claude/commands/` reasserts the directive; the handoff-message template inside `project-stage-close/SKILL.md` is itself caveman-shaped and includes an explicit "follow `caveman:caveman` skill rules" line that is forwarded verbatim to the next stage agent.
...
| 9 | **`tools/mcp-ia-server/tests/tools/build-registry.test.ts` failed with `25 !== 24`** when Stage 4 first ran `npm run validate:all`. The user's Stage 4 prompt mentions adding `@ia/rules/agent-output-caveman.md` to `CLAUDE.md`; the corresponding rule file `ia/rules/agent-output-caveman.md` was added to the tree between Stage 3 close (73 IA files) and Stage 4 start (74 IA files). The hard-coded count in the test fixture (`r.length === 24`, `rules.length === 12`) was authored before the new rule existed. | The test fixture asserts a frozen integer count of IA registry entries. Whenever a new spec or rule joins the tree, the count drifts and the test goes red. The drift is mechanical, not semantic — `buildRegistry()` itself works correctly. | **Fixed in-stage** by updating `tools/mcp-ia-server/tests/tools/build-registry.test.ts` to expect `25` total entries / `13` rules and to additionally assert that the `agent-output-caveman` rule key is present in the registry. Re-running `npm run validate:all` then returned `123/123` green. Out of scope strictly for Stage 4 (Stage 3 should arguably have updated the count when the file landed), but on the critical path for Stage 4 close. Going forward: when adding a new file to `ia/{specs,rules}/`, also update this test's hard-coded counts. Long-term fix candidate: replace the integer assertion with a `>= N` lower bound or pull the expected count from a generated index file. |
| 10 | **`tools/mcp-ia-server/tests/tools/build-registry.test.ts` count is fragile.** Stage 4 hit it once because Stage 3 added a rule file without updating the test. The hard-coded integer is a known fragility documented in §9 issue #9. | See issue #9. | **Mitigation deferred to a future stage / TECH item.** A `>= N` assertion or a derived count would prevent every future IA addition from going red. Filing this lesson here so the next maintainer is not surprised. |
| 11 | **§7b "Stage 4 caveman directive coverage" path-count claim is off by one.** §7b row text reads "returns all **17** paths" but the actual list is `5 agents + 5 commands + 6 skills = 16 paths`. Stage 4 close ran the grep, confirmed 16 hits, and noted the discrepancy here rather than silently fudging the count. §8 acceptance text describes the same surfaces without naming a number, so §8 is internally consistent. | Editorial typo in the spec body when §7b was authored. The math is straightforward: the 5 subagents and 5 slash commands are 10; the 6 skill recipes (`project-spec-kickoff`, `project-spec-implement`, `project-implementation-validation`, `agent-test-mode-verify`, `project-spec-close`, `project-stage-close`) are 6 more. Total 16. | **Fixed in-stage** by updating the §7b row text from "all 17 paths" to "all 16 paths" so the row matches the actual surfaces. The acceptance gate is the gross presence check (every surface carries a `caveman:caveman` directive), not the literal integer; the grep returns 16 paths and Stage 4 ships with all 16 covered. |
...
### Stage 4 lessons (2026-04-10)

- **`.claude/agents/*.md` uses Claude Code native frontmatter, NOT the four-field IA schema.** The native shape is `name`, `description`, `tools`, `model`. Subagent files live outside `ia/` and are consumed by the Claude Code host directly, not the territory-ia MCP server. `npm run validate:frontmatter` does not scan `.claude/`, so the four-field schema check does not apply. Future stages adding subagents must use the native shape, not the IA shape; mixing the two confuses the host parser. Captured as a §6 Decision Log row from 2026-04-10.
- **Subagent `tools` field is per-subagent allow-list semantics, NOT a wildcard.** Even though the session-level `permissions.allow` keeps the `mcp__territory-ia__*` wildcard (canonical project stance), each subagent's `tools` field enumerates the explicit subset its skill recipe needs. This scopes side effects: `verifier` cannot persist a journal entry; `spec-kickoff` cannot launch the Unity bridge; `closeout` cannot run `unity_bridge_command`. The wildcard at the **session** layer keeps per-call ergonomics; the explicit list at the **subagent** layer keeps blast radius. Both are correct; they operate at different scopes.
- **The model split lives in subagent frontmatter, not in a separate config.** `model: opus` (orchestrators: `spec-kickoff`, `spec-implementer`, `closeout`) vs `model: sonnet` (executors: `verifier`, `test-mode-loop`) is encoded directly in each subagent's frontmatter. No separate config table, no environment variable. Future tweaks (e.g. switching `closeout` to Sonnet for cost) are a one-line edit per subagent.
- **Output styles are markdown files with the same shape as subagents** — frontmatter (`name`, `description`) plus a body that the host treats as a system-prompt addition for any agent that cites the style. The Verification block JSON header lives in the output style, not in the subagent body, so a future change to the JSON contract is one edit in `.claude/output-styles/verification-report.md` instead of N edits across every subagent / skill / command that emits a Verification block. Same for the closeout digest.
- **Caveman directive enforcement is belt-and-suspenders by design** (per §6 caveman decision row): (a) every `.claude/agents/*.md` body, (b) every `ia/skills/{lifecycle}/SKILL.md` preamble, (c) every `.claude/commands/*.md` forwarded prompt, and (d) the `project-stage-close` handoff template that gets pasted into the next stage's fresh context window. Removing the directive from one surface still leaves three others enforcing it. Stage 4 verified all four layers in one pass and the grep gate counts 16 paths.
- **Slash command bodies are prompt templates the host expands and forwards to the subagent**, NOT a special "dispatch directive" format. The body contains "Use the **`X`** subagent... Forward the following prompt via the Agent tool with `subagent_type: \"X\"`...". The Claude Code host handles the actual subagent dispatch outside the in-session tool surface — the in-session `Agent` tool only sees built-in subagent types (`general-purpose`, `Explore`, `Plan`, `claude-code-guide`, `statusline-setup`); user-defined subagents under `.claude/agents/` are reachable only through the slash-command path the host parses. This is captured as both a §6 decision row and §9 issue prose because it has a real verification consequence (next bullet).
- **Stage 4 slash-command smoke is filesystem-level, with the interactive UI smoke deferred to the user** (same pattern as Stage 1 hook smoke and Stage 2 Cursor `alwaysApply` smoke). The agent that executes Stage 4 cannot dispatch into its own host UI to type `/verify`, `/kickoff TECH-85`, etc. The filesystem-level smoke (file exists, frontmatter parses, body carries caveman directive, expected subagent referenced from each command, no `Stage N stub` language) is sufficient for the structural acceptance; the interactive UI smoke is the user's last-mile confirmation. Document the deferral in the handoff so Stage 5 does not re-litigate it.
- **Hard-coded test fixtures count IA registry entries.** `tools/mcp-ia-server/tests/tools/build-registry.test.ts` asserts a frozen integer for `r.length` (total entries) and `rules.length`. Whenever a new spec or rule lands in `ia/{specs,rules}/`, the test goes red with `N+1 !== N` until someone updates the constant. Stage 4 hit it once because Stage 3 added `ia/rules/agent-output-caveman.md` without updating the test. Two ways to handle: (a) update the constant whenever a new file is added (current pattern, low ceremony, easy to forget), or (b) replace the integer with a `>= N` lower bound or pull the count from a generated index (better long-term, more intrusive change). Stage 4 chose (a) and filed the long-term improvement as §9 issue #10.
- **`project-stage-close` step 7 sanity check is the right place to enforce the canonical project stances.** `python3 -c '... assert d["defaultMode"]=="acceptEdits"; assert "mcp__territory-ia__*" in d["allow"]'` runs in one breath at every stage close and catches any accidental regression of the two §9 issue #4 fixes before the handoff is written. Stage 4 ran it and it passed. The same check should live in Stage 5's umbrella close so the umbrella close does not silently leave a regressed `.claude/settings.json` on `main`.
- **The handoff template inside `project-stage-close/SKILL.md` must itself include the caveman directive forwarded verbatim.** Stage 4 added the explicit "Follow `caveman:caveman` skill rules for all responses..." line at the top of the template, immediately after the H1, so any future stage close pasts it into the next agent's fresh context. Without that bake-in, the next stage runs in a clean context with no inherited SessionStart hook and falls back to verbose default phrasing — the very failure mode the §6 caveman decision row was written to prevent.
- **Stage 4 took zero hot-path code edits inside `tools/mcp-ia-server/src/`.** All deliverables are configuration / markdown additions plus one test fixture update. The MCP server schema cache invariant (caches at session start, restart needed for in-session validation) does not bite Stage 4 because no MCP tool descriptors changed. Stage 5 will be the first stage that touches `tools/mcp-ia-server/src/tools/` and will need to validate via tsx CLI script in the same session, not via MCP — this lesson generalized from Stage 2 / Stage 3 still applies.
