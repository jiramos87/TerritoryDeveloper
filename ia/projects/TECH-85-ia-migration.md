...
- [ ] Define and document the standard frontmatter schema in `ia/templates/frontmatter-schema.md`:
  ```yaml
  ---
  ...
  ---
  ```
- [ ] Add frontmatter to every file under `ia/specs/`, `ia/rules/`, `ia/skills/{name}/SKILL.md`, `ia/projects/`, `ia/templates/`.

#### Phase 3.2 — Frontmatter validator (advisory)

- [ ] Author `tools/mcp-ia-server/scripts/check-frontmatter.mjs` that fails if any `ia/**/*.md` is missing the four fields.
- [ ] Wire as `npm run validate:frontmatter` (advisory at first; CI promotion deferred).

#### Phase 3.3 — Densify root docs

- [ ] Densify `AGENTS.md`: convert prose paragraphs to bullet/table form, add a 3-line TL;DR at top, keep links but reduce noise; eliminate Cursor-specific phrasing in favor of "MCP-enabled host".
- [ ] Densify `BACKLOG.md`: shorten the lead paragraph, table-ify lane intros, drop redundant bold emphasis.
- [ ] Densify `docs/information-architecture-overview.md`: TL;DR at top, fewer prose paragraphs; update layer diagram to use `ia/`.
- [ ] Rewrite `CLAUDE.md` as a 40-line operative version with 5 sections: (1) what this repo is, (2) MCP first, (3) key files, (4) key commands, (5) where to find more.

#### Phase 3.4 — Verification policy consolidation

- [ ] Confirm `docs/agent-led-verification-policy.md` is the single canonical source.
- [ ] Reduce these to 5-line stubs pointing to it:
...
- [ ] Run `npm run generate:ia-indexes` again to capture frontmatter changes.
- [ ] Verification: `npm run validate:all` green; `npm run validate:frontmatter` green; manual review of densified docs.
- [ ] Run `project-stage-close` — handoff to Stage 4 agent.
...
| 7 | **The dev-time `tools/mcp-ia-server/data/spec-index.json` retains a literal `.cursor/specs/` reference** (line 12 — the H1 of `ia/specs/REFERENCE-SPEC-STRUCTURE.md` is still `# Reference spec structure — \`.cursor/specs/\``). Phase 2.4 sed pass intentionally excludes `ia/` content; the spec H1 is densification work for Stage 3 / Phase 3.3. | The spec body inside `ia/specs/REFERENCE-SPEC-STRUCTURE.md` was authored before the migration. Stage 2 only renames the namespace; it does not refresh spec bodies. The generated index file faithfully reflects the source. | **Deferred to Stage 3 / Phase 3.3** (densify root + reference docs). The §8 audit targets do not include `tools/mcp-ia-server/data/`, so this does not block Stage 2 close. |
