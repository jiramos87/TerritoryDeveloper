---
purpose: "TECH-723 — curate.py promote subcommand appending JSONL rows to curation/promoted.jsonl."
audience: both
loaded_by: ondemand
slices_via: none
...
task_key: T6.5.1
---
# TECH-723 — curate.py promote → promoted.jsonl
...
|---|------|-------|---------------------|
| 1 | Curator | Promote a good variant | CLI prints path; row appears in `promoted.jsonl` |
...
`curate.py` today is a light inspection/render CLI; no state-writing subcommands.

### 4.2 Systems map

- `tools/sprite-gen/src/curate.py` — CLI entry.
- `curation/promoted.jsonl` — new append-only log file.

### 4.3 Implementation investigation notes

Row-writer + measurement helpers here are shared by TECH-724 (reject); keep them in a module-level helper so TECH-724 reuses directly.
...
```bash
$ python3 -m src.curate promote renders/residential_small__variant3.png
[promote] appended row #7 → curation/promoted.jsonl
```

### 5.2 Architecture / implementation

- New `promote` subparser.
...
|-------------------|------------|---------------------|-------|
| Row appended | Python | `pytest tests/test_curate.py::test_promote_appends_row -q` | File gains one row |
| Schema populated | Python | `pytest tests/test_curate.py::test_promote_row_schema -q` | All fields non-null |
| Idempotency | Python | `pytest tests/test_curate.py::test_promote_history_preserved -q` | Re-invoke preserves prior rows |

## 8. Acceptance Criteria

- [ ] `promote <variant>` appends one JSON row to `curation/promoted.jsonl`.
...
Promote a rendered variant into a curator-approved JSONL log that downstream aggregators can tighten the signature envelope against.

### §Acceptance

- [ ] `python3 -m src.curate promote <variant>` exits 0 and appends exactly one row to `curation/promoted.jsonl`
...
|-----------|--------|----------|---------|
| test_promote_appends_row | rendered variant + empty log | file has exactly 1 row | pytest |
| test_promote_row_schema | rendered variant | all schema keys populated | pytest |
| test_promote_history_preserved | 2× invocations | 2 rows; first row byte-identical | pytest |
| test_promote_creates_log_file | no existing log | file created on first call | pytest |
...
def promote(variant_path: str) -> None:
    ...
    print(f"[promote] appended → {_PROMOTED}")
```
...
- `tools/sprite-gen/src/curate.py` — add `promote` subparser.

**Gate:**

```bash
cd tools/sprite-gen && python3 -m src.curate promote --help
```
...
```bash
cd tools/sprite-gen && python3 -m pytest tests/test_curate.py::test_promote_row_schema -q
```
...
- Same file — `_append_jsonl`; wire to `promote` entry.
