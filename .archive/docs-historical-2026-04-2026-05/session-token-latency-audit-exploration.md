...
| F4 | O4 | Unified `ia/state/runtime-state.json` (harness-agnostic; gitignored data file + MCP `runtime_state`) replacing scattered flat-file markers (`last-verify-exit-code`, `last-bridge-preflight-exit-code`, `.queued-test-scenario-id`). SessionStart emits a single deterministic preamble (feeds D2). Hooks append fields without naming contention. | 1 d | operationalises D2 | high |
...
2. ~~**Theme B ↔ MCP reshape sibling ordering.**~~ **Resolved (2026-04-19).** MCP plan Stage 5 T5.1 TECH-426 Done absorbed B2 entirely; row struck through + §Sibling master plan coordination documents it. Remaining Theme B items (B1, B3–B9) independent of MCP plan Stages 6–16, except B6 which stacks on Stage 9 T9.4 (noted in B6 row).
   - ~~**B1 primary decision (server-split vs `defer_loading`).**~~ **Resolved (2026-04-20) via TECH-527.** Chose two-server split (IA-core + bridge) over `defer_loading: true` per-tool. Landed in Stage 1.2 TECH-524 (extraction) + TECH-525 (`.mcp.json` wiring) + TECH-526 (CI test). `MCP_SPLIT_SERVERS` flag default `0` through Stage 1.2 (backward compat); Stage 1.3 T1.3.6 flips default to `1` after post-stage sweep (T1.3.5) confirms per-theme attribution correctness per NB-6. F7 (`defer_loading` tracking) remains the antidote-if-split-regresses safety valve; no action required while two-server path stays viable.
