...
- [x] Phase 1 — Bulk-terms handler + tests.

**Tasks:**

| Task | Name | Phase | Issue | Status | Intent |
|---|---|---|---|---|---|
| T1.1.1 | Bulk terms handler | 1 | **TECH-314** | Done | Extend `tools/mcp-ia-server/src/tools/glossary-lookup.ts` to accept `terms?: string[]` alongside `term?: string`. When `terms` present, fan out to per-term lookup, aggregate into `{ results: {[term]: GlossaryEntry}, errors: {[term]: { code, message }} }` + `meta.partial: { succeeded, failed }`. Single-`term` path returns existing shape via backward-compat branch. |
| T1.1.2 | Bulk terms tests | 1 | **TECH-315** | Done | Unit tests in `tools/mcp-ia-server/tests/tools/glossary-lookup.test.ts`: bulk happy path (all found), partial failure (one term not found → in `errors`, rest in `results`), single-`term` back-compat, empty `terms: []` → `{ results: {}, errors: {}, meta.partial: {succeeded:0,failed:0} }`. |
...
- [x] Phase 1 — Sidecar + handler extension.
- [x] Phase 2 — Tests + release prep.

**Tasks:**

| Task | Name | Phase | Issue | Status | Intent |
|---|---|---|---|---|---|
| T1.2.1 | Invariants-tags sidecar | 1 | **TECH-371** | Done (archived) | Author `tools/mcp-ia-server/data/invariants-tags.json` — array of `{ number: N, subsystem_tags: string[] }` for all 13 invariants + Guardrails rows (derive tags from `ia/rules/invariants.md` prose: HeightMap/Cell/roads/water/cliff/urbanization mentions). |
| T1.2.2 | Structured invariants handler | 1 | **TECH-372** | Done (archived) | Extend `tools/mcp-ia-server/src/tools/invariants-summary.ts` to load `invariants-tags.json`; accept `domain?: string` filter param (substring match against `subsystem_tags`); return `{ invariants: [{number, title, body, subsystem_tags, code_touches}], markdown?: string }`. `markdown` preserves existing prose for agents that still prefer text rendering. |
| T1.2.3 | Invariants tests | 2 | **TECH-373** | Done (archived) | Unit tests in `tools/mcp-ia-server/tests/tools/invariants-summary.test.ts`: `domain` filter match; `domain` matches nothing → `{ invariants: [], markdown: "" }` (not error); no `domain` → all 13 returned; `markdown` side-channel populated regardless of filter. |
| T1.2.4 | Release prep v0.6.0 | 2 | **TECH-374** | Done (archived) | Bump `tools/mcp-ia-server/package.json` `version` to `0.6.0`; add `CHANGELOG.md` entry: `v0.6.0 — Quick wins: glossary bulk-terms + structured invariants`. Advisory note: "tag this commit `mcp-pre-envelope-v0.5.0` for P2 rollback target". |
...
| Task | Name | Phase | Issue | Status | Intent |
|---|---|---|---|---|---|
| T2.1.1 | Envelope middleware | 1 | **TECH-388** | Done (archived) | Author `tools/mcp-ia-server/src/envelope.ts`: `ToolEnvelope<T>` discriminated union (`ok: true, payload: T, meta?` / `ok: false, error: {code, message, hint?, details?}`); `EnvelopeMeta` with `graph_generated_at?`, `graph_stale?`, `partial?: {succeeded, failed}`; `ErrorCode` enum (12 values from §3.1); `wrapTool<I,O>(handler: (input:I)=>Promise<O>)` that catches throws + converts to error envelope. |
| T2.1.2 | Caller allowlist | 1 | **TECH-389** | Done (archived) | Author `tools/mcp-ia-server/src/auth/caller-allowlist.ts`: per-tool allowlist map `Record<string, string[]>` covering all mutation + authorship tools from Steps 3–4 (pre-populate with known callers per §3.8); export `checkCaller(tool: string, caller_agent: string | undefined): void` — throws `{ code: "unauthorized_caller", message, hint }` if caller not in allowlist or allowlist missing `caller_agent`. |
| T2.1.3 | Envelope unit tests | 2 | **TECH-390** | Done (archived) | Tests in `tools/mcp-ia-server/tests/envelope.test.ts`: `wrapTool` happy path (`ok: true, payload`); envelope passthrough (no double-wrap); bare `Error` → `internal_error` (per TECH-388 Decision Log); typed throw `{code: "db_unconfigured", hint?, details?}` preserves code + optional fields; `meta` passthrough. |
| T2.1.4 | Allowlist unit tests | 2 | **TECH-391** | Done (archived) | Tests for `checkCaller`: authorized caller → no throw; unauthorized caller → `unauthorized_caller`; `caller_agent` undefined → `unauthorized_caller`; tool not in map (read-only) → no throw (allowlist only gates mutation/authorship tools; read tools bypass). |
...
| Task | Name | Phase | Issue | Status | Intent |
|---|---|---|---|---|---|
| T2.2.1 | Wrap spec tools | 1 | **TECH-398** | Done (archived) | Wrap `list-specs.ts`, `spec-outline.ts`, `spec-section.ts`, `spec-sections.ts` in `wrapTool`; add `payload.meta: { section_id, line_range, truncated, total_chars }` to `spec-section` success response; convert `spec_not_found` / `section_not_found` to typed `ErrorCode`. |
| T2.2.2 | Wrap rule + router tools | 1 | **TECH-399** | Done (archived) | Wrap `list-rules.ts`, `rule-content.ts`, `router-for-task.ts` in `wrapTool`; add new `rule_section` tool (symmetric to `spec_section`, canonical params `{ rule, section, max_chars }`) in `rule-content.ts` alongside existing `rule_content`; register in MCP server index. |
| T2.2.3 | Wrap glossary tools | 2 | **TECH-400** | Done (archived) | Wrap `glossary-discover.ts`, `glossary-lookup.ts` (including bulk-`terms` path from Stage 1.1) in `wrapTool`; ensure `meta.graph_generated_at` + `meta.graph_stale` preview fields flow through envelope `meta` (full freshness logic in Stage 3.3). |
| T2.2.4 | Wrap invariant tools | 2 | **TECH-401** | Done (archived) | Wrap `invariants-summary.ts` (structured response from Stage 1.2) and `invariant-preflight.ts` in `wrapTool`; convert hardcoded section-cap constants to `INVARIANT_PREFLIGHT_MAX_SECTIONS` / `INVARIANT_PREFLIGHT_MAX_CHARS` env vars with existing defaults. |
| T2.2.5 | Wrap backlog tools | 3 | **TECH-402** | Done (archived) | Wrap `backlog-issue.ts`, `backlog-search.ts` in `wrapTool`; `issue_not_found` → `{ ok: false, error: { code: "issue_not_found", hint: "Check ia/backlog/ and ia/backlog-archive/" } }`. |
| T2.2.6 | Wrap DB-coupled tools | 3 | **TECH-403** | Done (archived) | Wrap `city-metrics-query.ts`, `project-spec-closeout-digest.ts`, `project-spec-journal.ts` (all 4 journal ops) in `wrapTool`; `db_unconfigured` branch → `{ ok: false, error: { code: "db_unconfigured", hint: "Start Postgres on :5434" } }` uniformly across all four. |
| T2.2.7 | Wrap bridge tools | 4 | **TECH-404** | Done (archived) | Wrap `unity-bridge-command.ts`, `unity-bridge-lease.ts`, `unity-bridge-get.ts` (via `unity_bridge_get` / `unity_compile` kinds), `unity-compile.ts` in `wrapTool`; timeout path: inject `error.details = { command_id, last_output_preview }` before wrapping; `db_unconfigured` → `{ ok: false, error: { code: "db_unconfigured" } }`. |
| T2.2.8 | Wrap Unity analysis tools | 4 | **TECH-405** | Done (archived) | Wrap `findobjectoftype-scan.ts`, `unity-callers-of.ts`, `unity-subscribers-of.ts`, `csharp-class-summary.ts` in `wrapTool`; no-results path returns `ok: true, payload: { matches: [] }` (not error); parse failure → `ok: false, error: { code: "invalid_input" }`. |
...
### Step 5 — Critical Misses: Authoring Parity + Atomicity + Dry-run (post-plan review addendum)

**Status:** Draft (tasks _pending_ — not yet filed)

**Backlog state (Step 5):** 0 filed

**Origin:** Added in post-plan conversational review (2026-04-18) after identifying three critical gaps not covered by the original exploration's §3.6 (mutation) + §3.7 (authorship) surfaces. Extension follows existing caller-allowlist + guarded-authorship patterns — no contradiction with locked decisions. If formal traceability needed, mirror into `docs/mcp-lifecycle-tools-opus-4-7-audit-exploration.md` under a new `## Design Expansion — post-plan addendum` block.

**Objectives:** Close three critical gaps surfaced in post-plan review: (1) **master-plan authoring parity** — proposal guards glossary/spec/rule authorship but leaves highest-churn orchestrator surface (`master-plan-new`, `master-plan-extend`, `stage-decompose` skills) as raw `Write`/`Edit`; (2) **transactional batching** for multi-step mutations — `stage-file` files N issues + N yaml records + N spec stubs + BACKLOG regen + orchestrator task-table update; half-filed state is a known pain today; (3) **dry-run preview mode** across all mutation + authorship tools so agents can show user the planned diff before applying. Ships as additive release `v1.1.0`.

**Exit criteria:**

- `master_plan_create`, `master_plan_step_append`, `stage_decompose_apply` all `caller_agent`-gated; enforce `project-hierarchy` cardinality (≥2 phases/stage, ≥2 tasks/phase) before write; validate target path under `ia/projects/` (invariant #12).
- `mutation_batch({ ops, mode })` wraps N mutation / authorship calls atomically; `all_or_nothing` restores file snapshots on any op failure; `best_effort` returns per-op results under partial-result schema.
- `dry_run?: boolean` available on every mutation + authorship tool (Stages 4.1, 4.2, 5.1, 5.2); returns `payload.diff` (unified diff) + `affected_paths` without writing; `caller_agent` gate still runs first.
- `tools/.mutation-batch.lock` distinct lockfile for concurrent batch serialization (per invariants guardrail on distinct lockfiles).
- Caller skill updates: `master-plan-new` → `master_plan_create`; `master-plan-extend` → `master_plan_step_append`; `stage-decompose` → `stage_decompose_apply`; `stage-file` + `closeout` wrap multi-file mutations in `mutation_batch`.
- Snapshot tests for diff output per tool; cardinality-violation tests per authoring tool; rollback verified via pre/post snapshot equality.
- `tools/mcp-ia-server/package.json` at `1.1.0`; `CHANGELOG.md` entry.

**Art:** None.

**Relevant surfaces (load when step opens):**
- Step 2 exit: `tools/mcp-ia-server/src/envelope.ts` + `caller-allowlist.ts` (add new agents `master-plan-new`, `master-plan-extend`, `stage-decompose` to per-tool allowlist map)
- Step 3 exit: `tools/mcp-ia-server/src/parser/orchestrator-parser.ts` (author inverse `serialize()` here; dual-use parser/serializer)
- Step 4 exit: mutation + authorship tools shipped (wire `dry_run` + `mutation_batch` compat)
- `ia/rules/project-hierarchy.md` — cardinality source (≥2 phases/stage, ≥2 tasks/phase)
- `ia/rules/invariants.md` — guardrail on distinct lockfiles (mutation-batch lockfile distinct from id-counter / closeout / materialize-backlog)
- `ia/skills/master-plan-new/SKILL.md`, `ia/skills/master-plan-extend/SKILL.md`, `ia/skills/stage-decompose/SKILL.md`, `ia/skills/stage-file/SKILL.md`, `ia/skills/project-spec-close/SKILL.md` — caller update targets

---

#### Stage 5.1 — Master-plan Authoring Tools
