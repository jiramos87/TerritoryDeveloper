...
**Purpose:** Map ideas and examples from [`ia-driven-dev-territory-prompt.md`](ia-driven-dev-territory-prompt.md) to cases that **gain materially** from a **PostgreSQL**-style backend, using [`.cursor/projects/TECH-44a.md`](../.cursor/projects/TECH-44a.md) patterns (**B1** row + **JSONB**, **B3** idempotent patch **envelope**, **P5** large-document / streaming guidance). This file is **exploratory narrative**; **normative pattern text** lives in **TECH-44a** §5; the **TECH-44** extension mapping (**E1**–**E3**) lives in **[TECH-44](../.cursor/projects/TECH-44.md) section 3**.

**Relationship to TECH-44 program (exact vs mapped):**

- **TECH-44a** in-scope deliverables (its section 8 acceptance) are **documentation only**: **B1**, **B3**, **P5**, and the SQL vs **Interchange JSON** naming table — illustrative SQL/JSON, **not** the full migration roadmap (**TECH-44b** ships the **IA** DB).
...
- **New capabilities** that need **extra normative rules** (lifecycle beyond **B3**, staging tables) are tracked as **E1**–**E3** in [**TECH-44** charter section 3](../.cursor/projects/TECH-44.md): **E1** → **TECH-44c**; **E2** → **[TECH-53](../BACKLOG.md)**; **E3** → **[TECH-54](../BACKLOG.md)** (**TECH-53**/**TECH-54** are backlog-only, no project specs). Promoting scope means updating **TECH-44a** §5 and/or refining those backlog rows.

**Boundaries (from TECH-44a / glossary):** Player **Save data** (`GameSaveData` / **CellData** / **WaterMapData**, **Load pipeline**) stays on the Unity **persistence-system** path until a **dedicated migration issue**. Backend rows hold **interchange**-shaped or **sync** payloads (**Interchange JSON (artifact)**), analytics, IA caches, or dev telemetry — not a silent replacement for **Save data**.

**MCP context used:** `backlog_issue` (**TECH-44b**, **TECH-44a** alignment), `glossary_lookup` (**Save data**), `glossary_discover` (**Interchange JSON (artifact)**).

---

## 1. What TECH-44a is optimizing for (short recap)
...
| Source in prompt doc | Idea / example | Backend benefit | Primary TECH-44a hook |
|----------------------|----------------|-----------------|------------------------|
...
The table below mirrors **[TECH-44](../.cursor/projects/TECH-44.md) section 3**. When prioritizing backend work, update the **TECH-44** charter first; keep this section aligned when possible.
...
| Balancing, **FEAT-47** / **FEAT-48** studies | *(charter narrative)* | **B1** today; promote field conventions via **TECH-44a** **Decision Log** |
...
| **Simulation / snapshot / CI warehouse / push** (unscheduled) | **AUTO** tick ledgers, **`world_snapshot_dev`** catalogs, Unity/CI run warehouses, WebSocket delivery — useful at scale; no **TECH-44** extension id. | New **TECH-**/**FEAT-** issue + **TECH-44a** updates if normative rules are needed. |
...
| **Multipolar growth experiments** | **FEAT-47**-shaped parameters and measured outcomes in **JSONB** for offline analysis; aligns with **TECH-44a** forward pointer to **FEAT-47** / **FEAT-48** field names. | **B1**; promote conventions via **TECH-44a** **Decision Log** |
...
1. [TECH-44](../.cursor/projects/TECH-44.md) — program charter, **E1**–**E3** mapping, **44a**/**b**/**c** order; **TECH-53**/**TECH-54** on [`BACKLOG.md`](../BACKLOG.md).  
2. [TECH-44a](../.cursor/projects/TECH-44a.md) — **B1** / **B3** / **P5**, naming (section 5.4).  
3. [TECH-44b](../.cursor/projects/TECH-44b.md) — **IA** Postgres milestone.  
4. [TECH-44c](../.cursor/projects/TECH-44c.md) — **E1** repro registry.  
...
*Exploration and mapping; **TECH-44a** owns normative patterns; **TECH-44** owns the **E1**–**E3** mapping. When implementing, record contracts in **Decision Log** / **TECH-44b** scope and keep **Save data** / **Load pipeline** boundaries explicit.*
