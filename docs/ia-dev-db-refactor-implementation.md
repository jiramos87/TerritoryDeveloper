...
- **Smoke task: TECH-858** (Stage 5 sentinel filed in Step 6). Skill body executed manually in this session (not via subagent dispatch — branch guardrail + stale MCP schema in live session prevented in-session subagent spawn; same pattern as Step 6 smoke).
  ...
  - Phase 7 — `task_spec_section_write({task_id: "TECH-858", section: "Plan Digest", content: <body>, change_reason: "stage-authoring smoke Step 7 — TECH-858 §Plan Digest direct write (no §Plan Author intermediate)", actor: "stage-authoring-smoke", git_sha: "d73a68e4646104e2f1ec94a9c0e7e4024d06288f"})` → `{ok: true, history_id: 17, updated_at: "2026-04-24T20:02:11.864Z"}`. Filesystem mirror at the flat task spec path was updated in same pass at smoke time (idempotent §Plan Digest block replace) — flat specs since deleted in Step 9.5; DB body is now the sole source of truth.
...
- **Surprises / iteration signals:**
  ...
  5. **TECH-762 readback proves rubric drift, not authoring fault.** Ran `mechanicalization_preflight_lint` on **TECH-762** (canonical TECH-776 reference example shipped on `main`; flat task spec since deleted in Step 9.5 — body now lives in DB) — also returns `{picks: partial, validators: partial}`. Confirms preflight FAIL is structural to the rubric, not a regression introduced by this smoke. Logged as TECH-776 scope-expansion lessons; hatch widening proposal in finding #1 above.
