...
- Stage skeletons / "decomposition deferred" blocks — `ship-plan` decomposes ALL Stages at author time via `master_plan_bundle_apply`. Lazy-decompose fires only when new Stage skeleton intentionally version-bumped by `ship-plan --version-bump` with deferred marker — expanded in place during next `ship-plan` pass.
