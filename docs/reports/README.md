...
| File | Source | Refresh |
|------|--------|---------|
| `ui-inventory-as-built-baseline.json` | **`editor_export_ui_inventory.document`** (Postgres) or **Territory Developer → Reports → Export UI Inventory (JSON)** | After **MainMenu** / **CityScene** UI hierarchy changes, or when updating **`ia/specs/ui-design-system.md`** **as-built** tables. |
