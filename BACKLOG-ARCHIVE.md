...
- [x] **TECH-465** — Preview deploy + visual confirm + M5 flip (Stage 6 T6.4) (2026-04-19)
  ...
  - Spec: (removed after closure)
  - Notes: User ran `npm run deploy:web:preview` manually; visually confirmed Stage→Task 2-level tree on `/dashboard` + `/dashboard/releases/*/progress` across migrated master plans; no Phase rows present. Migration JSON `M5.status` already flipped to `done` (Web dashboard: parser + PlanTree collapse). Closes Stage 6; unblocks Stage 7 (skills + agents + commands rewrite).
  - Acceptance: Preview reachable; 2-level render confirmed; `M5.status == "done"`.
  - Depends on: **TECH-464** (archived — web validate green)
  - Related: TECH-462, TECH-463, TECH-464

- [x] **TECH-464** — Web validate + type-check post Phase-fold (Stage 6 T6.3) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling / web)
  - Files: `web/`
  - Spec: (removed after closure)
  - Notes: `cd web && npm run validate:web` (= lint + typecheck + build) green post Phase-type removal. No TypeScript errors from dropped `PhaseNode`.
  - Acceptance: `validate:web` exit 0.
  - Depends on: **TECH-463**
  - Related: TECH-462, TECH-463, TECH-465

- [x] **TECH-463** — Update plan-tree + plan-loader for Stage→Task 2-level render (Stage 6 T6.2) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling / web)
  - Files: `web/lib/plan-tree.ts`, `web/lib/plan-loader.ts`, `web/app/dashboard/**`
  - Spec: (removed after closure)
  - Notes: Removed Phase header rendering row in `plan-tree.ts`; tasks render directly under Stage. Task Status column preserves `_pending_ / Draft / In Review / In Progress / Done (archived)`. Audited `web/app/dashboard/**/page.tsx` for Phase-specific props — none remained.
  - Acceptance: `PlanTree` renders Stage→Task; no Phase row.
  - Depends on: **TECH-462**
  - Related: TECH-464, TECH-465

- [x] **TECH-462** — Update plan-loader-types + parser for Phase-level drop (Stage 6 T6.1) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling / web)
  - Files: `web/lib/plan-loader-types.ts`, `web/lib/plan-parser.ts`, `web/lib/releases.ts`, `web/lib/plan-loader.ts`
  - Spec: (removed after closure)
  - Notes: Removed `PhaseNode` type; `StageNode` now holds `tasks: TaskNode[]` directly. `plan-parser.ts` no longer detects `#### Phase N` / `- [ ] Phase N` rows; tasks re-grouped directly under parent Stage. `releases.ts` + `plan-loader.ts` audited for Phase field refs — none remaining.
  - Acceptance: Types reflect 2-level; parser tests pass.
  - Related: TECH-463, TECH-464, TECH-465

- [x] **TECH-467** — Delete `verification-reminder.sh` Stop hook (D1 quick-win, session-token-latency) (2026-04-19)
  - Type: chore / hook cleanup
  - Files: `.claude/settings.json`, `tools/scripts/claude-hooks/verification-reminder.sh`
  - Spec: (removed after closure)
  - Notes: Theme-0 D1 quick-win from `docs/session-token-latency-audit-exploration.md` §Approach B. Stop hook fired unconditional verification reminder every session end — pure noise + 1 h cache-suffix drag; no gate function (`/verify-loop` + CI enforce verification policy independently). Deleted `hooks.Stop[0]` entry from `.claude/settings.json` + `git rm`'d `tools/scripts/claude-hooks/verification-reminder.sh`. `.claude/` residual refs: zero. Historical prose refs in `docs/session-token-latency-*.md` + `docs/ai-mechanics-audit-*.md` retained (audit paper trail). FREEZE-safe standalone delete.
  - Acceptance: Stop hook entry removed; script `git rm`'d; no residual refs in `.claude/`; `validate:all` green (TECH-468..480 yaml-missing-spec errors pre-existing, unrelated).

- [x] **TECH-466** — Flip `DEBUG_MCP_COMPUTE=1` on by default in `.mcp.json` env (2026-04-19)
  - Type: chore / config
  - Files: `.mcp.json`
  - Spec: (removed after closure)
  - Notes: Flipped `DEBUG_MCP_COMPUTE: "1"` on by default under `mcpServers.territory-ia.env` in `.mcp.json` alongside existing `REPO_ROOT`. Stderr stdio-safe (MCP spec: stdout = protocol, stderr = free); surfaces per-tool compute + cold-start timing on every fresh Claude Code session. Source: `docs/session-token-latency-audit-exploration.md` §B7 (Theme 0 ship-order #1 of 5 — B7 → D1 → E1 → E2 → D3). Zero runtime C# impact; no invariants touched. NB-2 deferred (audit recommends unconditional).
  - Acceptance: `.mcp.json` env carries both keys; JSON valid; `validate:all` green.

- [x] **TECH-461** — Register `plan_apply_validate` + schema-cache restart + M4 flip (Stage 5 T5.4) (2026-04-19)
  ...
  - Spec: (removed after closure)
  - Notes: Registered `plan_apply_validate` in MCP tool index (`src/index.ts`) alongside sibling lifecycle tools (`registerMasterPlanLocate`, `registerMasterPlanNextPending`). Handler import + registration call added post-`registerMasterPlanNextPending`. `npm run validate:all` green (379 unit tests pass including 7 `plan-apply-validate.test.ts` branches). Flipped `ia/state/lifecycle-refactor-migration.json` M4 status → `done`. Closes Stage 5.
  - Acceptance: Tool registered in MCP index; `validate:all` green; M4 `done`; Stage 5 Final.
  - Depends on: **TECH-460** (archived — handler authored)
  - Related: TECH-458, TECH-459

- [x] **TECH-460** — Author `plan_apply_validate` MCP tool (Stage 5 T5.3) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling / MCP)
  - Files: `tools/mcp-ia-server/src/tools/plan-apply-validate.ts`, `tools/mcp-ia-server/tests/tools/plan-apply-validate.test.ts`
  - Spec: (removed after closure)
  - Notes: Authored `plan_apply_validate(section_header, target_path)` handler with pure-core `validatePlanApply` + registration shell. Non-throwing contract — fs/read errors surface in `error` field with `ok:false`. Heading regex `^## {exact-title}$`; next-section boundary regex `^#{1,2}\s+\S`; tuple heuristic: line starts with `-` or `{`, or contains `operation:`; fenced-code `\`\`\`` toggle excludes tuples inside code blocks. 7 unit tests cover missing heading / empty section / valid tuples / mixed-shape + fenced-code / fs read error / empty section_header / empty target_path. Registration in `src/index.ts` deferred to TECH-461 (T5.4).
  - Acceptance: Handler file + tests present; 3-branch coverage green; `npm run validate:all` exit 0.
  - Depends on: **TECH-448** (archived — plan-apply-pair-contract.md authored, tuple shape reference)
  - Related: TECH-458, TECH-461

- [x] **TECH-459** — Schema cache restart + smoke tests post Phase-param drop (Stage 5 T5.2) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling / MCP)
  - Files: `tools/mcp-ia-server/`
  - Spec: (removed after closure)
  - Notes: Handler unit tests green (`npm run test:ia`); `router_for_task` `lifecycle_stage` enum accepts 11 pair/enrich values post-respawn; no Phase-era residue in touched handlers. Live smoke call w/ `lifecycle_stage: plan_review` returned non-error 2026-04-19 — cache confirmed fresh after host relaunch. Unblocks T5.3 / T5.4.
  - Acceptance: Handler unit tests green; `validate:all` green; no Phase-era residue; live smoke accepts `plan_review`.
  - Depends on: **TECH-458** (archived — handler changes landed)
  - Related: TECH-460, TECH-461

- [x] **TECH-458** — Drop Phase params from MCP handlers + update `lifecycle_stage` enum + closeout-digest new sections (Stage 5 T5.1) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling / MCP)
  - Files: `tools/mcp-ia-server/src/tools/router-for-task.ts`, `spec-section.ts`, `backlog-issue.ts`, `project-spec-closeout-digest.ts`
  - Spec: (removed after closure)
  - Notes: Guard-grep confirmed zero `phase` / `parent_phase` input params on 4 target handlers (residue lives in parser + master-plan-locate, out of scope). Added Zod `LifecycleStage` enum (11 values) as optional passthrough input on `router_for_task`. Extended `project_spec_closeout_digest` H2 reader w/ `audit`, `code_review`, `code_fix_plan`, `closeout_plan` section keys (empty string on missing, consistent w/ existing behavior). Unit tests cover enum accept/reject + 4-section digest read. Stage 5 Phase 1 exit met; unblocks T5.2 schema cache restart.
  - Acceptance: No Phase params; 11-value enum; digest reads 4 new sections; `validate:all` green.
  - Depends on: **TECH-457** (archived — BACKLOG regen clean)
  - Related: TECH-459, TECH-460, TECH-461

- [x] **TECH-457** — BACKLOG regen + `validate:all` + flip M3 done (Stage 4 T4.4) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling)
  - Files: `BACKLOG.md`, `BACKLOG-ARCHIVE.md`, `tools/scripts/materialize-backlog.sh`, `ia/state/lifecycle-refactor-migration.json`
  - Spec: (removed after closure)
  - Notes: `bash tools/scripts/materialize-backlog.sh` regenerated `BACKLOG.md` + `BACKLOG-ARCHIVE.md` from post-TECH-456 yaml state; no Phase column in view. `npm run validate:all` exit 0. Migration JSON `M3.status` already `done` (flipped on TECH-456 close). Stage 4 green-bar; unblocks Stage 5 (MCP server drop-Phase).
  - Acceptance: Views regenerated; `validate:all` exits 0; `M3.status` == `done`; ready for Stage 5.
  - Depends on: **TECH-456** (archived — yaml schema clean)
  - Related: TECH-454, TECH-455, TECH-456

- [x] **TECH-456** — Drop `phase` field from all backlog yaml records (Stage 4 T4.3) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling)
  - Files: `ia/backlog/`, `ia/backlog-archive/`, `tools/mcp-ia-server/src/parser/`, `ia/templates/frontmatter-schema.md`
  - Spec: (removed after closure)
  - Notes: Stripped `phase:` from every open + archived backlog yaml via migration loop; `parent_stage:` populated across yamls; parser + frontmatter-schema allowlist updated (backlog-parser.ts + backlog-yaml-loader.ts); `id:` counter byte-identical. `M3.yaml` per-file entries all `done`. `npm run validate:all` exit 0.
  - Acceptance: No `phase:` field anywhere; `parent_stage:` correct; parser + schema allowlist clean; `id:` counter untouched; `M3.yaml` all `done`.
  - Depends on: **TECH-455** (archived — spec-side fold validated clean)
  - Related: TECH-454, TECH-455, TECH-457

- [x] **TECH-455** — Validate dead-spec + spec frontmatter post-fold (Stage 4 T4.2) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling)
  - Files: `ia/projects/`, `tools/scripts/validate-dead-project-specs.ts`, `tools/scripts/validate-frontmatter.ts`
  - Spec: (removed after closure)
  - Notes: `npm run validate:dead-project-specs` exit 0 (no orphan specs). `npm run validate:frontmatter` exit 0 (advisory mode; pre-existing `slices_via` warnings in skill files — orthogonal to TECH-454 fold). `grep -r parent_phase ia/projects/` zero hits (TECH-454 acceptance pre-met; field never shipped in legacy template). Hard gate for Stage 4 Phase 1 exit = grep pass, not validator output.
  - Acceptance: `validate:dead-project-specs` exits 0; `validate:frontmatter` exits 0; `grep -r parent_phase ia/projects/` zero hits; orphan-spec decisions logged (none flagged).
  - Depends on: **TECH-454** (archived — parent_phase fold done)
  - Related: TECH-454, TECH-456

- [x] **TECH-454** — Fold parent_phase frontmatter from open project specs (Stage 4 T4.1) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling)
  - Files: `ia/projects/`, `ia/state/lifecycle-refactor-migration.json`
  - Spec: (removed after closure)
  - Notes: Acceptance pre-met — `parent_phase:` never present in legacy open specs (pre-refactor template never shipped field). Stage 2.1 template already uses `parent_plan` + `task_key`. M3.specs JSON had 89 pending entries; flipped `done` programmatically since no file edits needed. `parent_stage:` injection skipped vs original acceptance — current template derives step/stage from `task_key` string, not stored as separate frontmatter row.
  - Acceptance: `grep -r parent_phase ia/projects/` zero hits; M3.specs per-file all `done`; `validate:all` + `validate:dead-project-specs` green.
  - Depends on: **TECH-453** (archived — Stage 3 Final)
  - Related: TECH-443, TECH-455

- [x] **TECH-453** — Post-batch validate:all + frontmatter sweep + flip migration JSON M2 done (Stage 2.1 Phase 2) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling)
  - Files: `ia/state/lifecycle-refactor-migration.json`, `ia/projects/`
  - Spec: (removed after closure)
  - Notes: Post-batch validation sweep after TECH-452 migrated all 17 master plans. `npm run validate:all` (11-step chain): steps 1–9 green with zero tooling changes — validators already tolerant of new `T{stage}.{k}` task-id format. `validate:web` ESLint failed on `web/design-refs/step-8-console/src/console-app.jsx` (pre-existing, TECH-411 reference material, not production) — fixed by adding `design-refs/**` to `web/eslint.config.mjs` globalIgnores. `validate:frontmatter` green (advisory mode; 33 advisory warnings on master plans + template expected — master plans don't use spec frontmatter format). Flipped `phases.M2.status` `pending` → `done`; all M2 per-file entries already `done`.
  - Acceptance: `validate:all` exit 0; `validate:frontmatter` exit 0; migration JSON `phases.M2.status == "done"`; M2 per-file entries unchanged.
  - Depends on: **TECH-452** (archived — batch migration complete)
  - Related: —

- [x] **TECH-450** — Author migrate-master-plans.ts transform script (Stage 2.1 Phase 1) (2026-04-19)
  - Type: tech (IA infrastructure / lifecycle tooling)
  - Files: `tools/scripts/migrate-master-plans.ts`, `ia/state/lifecycle-refactor-migration.json`, `ia/state/pre-refactor-snapshot/`
  - Spec: (removed after closure)
  - Notes: Authored `tools/scripts/migrate-master-plans.ts` TypeScript transform (run via `npx tsx`). Reads `ia/state/lifecycle-refactor-migration.json` M2.per-file; skips `done` files. Reads from `ia/state/pre-refactor-snapshot/{relative-path}` (never current). Line-scanner parse: maps old Step+Stage pair → new Stage (name `"{Step name} / {Stage name}"`); merges Phase exit bullets into parent Stage Exit; strips Phase headings + `Phase` column from task-table; renumbers `T{N}.{M}.{k}` → `T{stage_seq}.{k}` preserving Issue + Status verbatim. Atomic tempfile + `fs.rename` for both plan and migration JSON. `--only` / `--dry-run` / `--help` flags. Idempotent on `done` files. Foundational tool — canary (TECH-451) + batch (TECH-452) consume it.
  - Acceptance: `npx tsc --noEmit` clean; `--help` exit 0; `--dry-run --only blip-master-plan.md` emits transformed plan to stdout with Phase column absent + task ids renumbered; no new npm deps; atomic writes verified by code inspection.
  - Depends on: none (Stage 1.2 Final; foundational templates + rules ready)
  - Related: TECH-443, TECH-444, TECH-446
...
- [x] **BUG-57** — `/closeout` must sync step/stage header Status prose from task-table ground truth when last task archives (2026-04-17)
  - Type: bug (lifecycle tooling / orchestrator sync)
  - Files: `ia/skills/project-spec-close/SKILL.md`, `ia/skills/project-stage-close/SKILL.md`, `.claude/agents/closeout.md`, `tools/mcp-ia-server/src/parser/master-plan-header-sync.ts`, `tools/mcp-ia-server/tests/parser/master-plan-header-sync.test.ts`
  - Spec: (removed after closure)
  - Notes: Header-sync helper rewrites `**Status:**` + `**Backlog state ...**` paragraphs under step/stage headings from task-table ground truth — terminal → `Final`; intermediate → `In Progress — {first-open}` + `k filed`; idempotent. Wired into `project-spec-close` step 6 + `project-stage-close` step 1 + `closeout` agent seed. Fixture cases (a)–(d) under parser tests.
  - Acceptance: terminal flip → `Final`; intermediate refresh updates `k filed` + suffix; idempotent; fixture (a)–(d) green; `npm run validate:all` green.
  - Related: TECH-322

- [x] **TECH-326** — Implement `reserve_backlog_ids` MCP tool (Stage 1.2 Phase 2) (2026-04-17)
  - Type: infrastructure / MCP tooling
  - Files: `tools/mcp-ia-server/src/tools/reserve-backlog-ids.ts`, `tools/mcp-ia-server/src/index.ts`
  - Spec: (removed after closure)
  - Notes: Shipped `reserve_backlog_ids` MCP tool (IP3). Handler spawns `tools/scripts/reserve-id.sh {prefix} {count}` via `node:child_process` w/ `cwd: REPO_ROOT`; parses stdout → `{ ids: string[] }`; non-zero exit or parse mismatch → structured `RESERVE_FAILED` w/ captured stderr. Input via zod (prefix enum + count 1..50). `buildScriptEnv()` patches PATH w/ Homebrew util-linux Cellar so `flock` resolves on npm test shell (macOS). Registered alongside `registerBacklogRecordValidate`. Preserves invariant #13 (script-backed counter mutation). Unblocks `stage-file` / `project-new` MCP migration.
  - Acceptance: tool registered; single + batch work; counter advances; input validation rejects bad prefix / count > 50; `RESERVE_FAILED` carries stderr; tests use `IA_COUNTER_FILE` temp override; `validate:all` green.
  - Related: TECH-327, TECH-324

- [x] **TECH-327** — Concurrency test for `reserve_backlog_ids` (Stage 1.2 Phase 2) (2026-04-17)
  - Type: infrastructure / MCP tooling
  - Files: `tools/mcp-ia-server/tests/tools/reserve-backlog-ids.test.ts`
  - Spec: (removed after closure)
  - Notes: `C1` test block in `reserve-backlog-ids.test.ts` reuses `makeTempCounter` + `runScript` + `readCounter`; spawns 8 parallel `reserve-id.sh TECH 2` via `Promise.all` against shared tmp counter + lockfile; asserts 16 ids unique, monotonic `[1..16]`, counter === 16. Skip-with-warn when `flock` missing (mirrors script-layer DX). Closes Phase 2 of Stage 1.2.
  - Acceptance: 16 distinct ids; counter advances exactly 16; tmpdir + env override isolates from real counter; `validate:all` green.
  - Related: TECH-326

- [x] **TECH-324** — Implement `backlog_record_validate` MCP tool (Stage 1.2 Phase 1) (2026-04-17)
  - Type: infrastructure / MCP tooling
  - Files: `tools/mcp-ia-server/src/tools/backlog-record-validate.ts`, `tools/mcp-ia-server/src/index.ts`
  - Spec: (removed after closure)
  - Notes: Shipped MCP handler delegating to `validateBacklogRecord` shared core (TECH-323). Empty `yaml_body` short-circuits to `invalid_input` (no throw). Wrapped in `runWithToolTiming`. Registered alphabetically near `registerBacklogIssue`. Unblocks TECH-325 fixture tests.
  - Acceptance: tool registered + boots clean; good record → `ok: true`; bad record → shared-core rule ids; empty body → `invalid_input`; `npm run validate:all` green.
