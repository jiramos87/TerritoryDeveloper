/**
 * Button list + create (TECH-1885 / Stage 8.1).
 *
 *  GET  /api/catalog/buttons?status=active|retired|all&limit=50&cursor=...
 *  POST /api/catalog/buttons body: CatalogButtonCreateBody
 *
...
import {
  createButtonSpine,
  listButtonsSpine,
  type ButtonSpineListFilter,
} from "@/lib/catalog/button-spine-repo";
import type { CatalogButtonCreateBody } from "@/types/api/catalog-api";
...
export async function GET(request: NextRequest) {
  ...
  const filter = statusRaw as ButtonSpineListFilter;
  ...
  try {
    const out = await listButtonsSpine({ filter, limit, cursor: cursorRaw });
    ...
    if (e instanceof Error && e.message === "DATABASE_URL not set — required for DB access.") {
      return catalogJsonError(500, "internal", "Database not configured", { logContext: "buttons-list" });
    }
    return responseFromPostgresError(e, "Button list query failed");
  }
}
...
const wrappedPost = withAudit(async (request, { emit, sql }) => {
  ...
  const result = await createButtonSpine(body as CatalogButtonCreateBody, sql);
  ...
  await emit("catalog.button.create", "catalog_entity", result.data.entity_id, {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (e instanceof Error && e.message?.startsWith("conflict:")) {
      ...
      return catalogJsonError(409, reason === "duplicate_slug" ? "unique_violation" : "conflict", `Button ${reason}`);
    }
    if (e instanceof Error && e.message === "DATABASE_URL not set — required for DB access.") {
      return catalogJsonError(500, "internal", "Database not configured", { logContext: "buttons-post" });
    }
    return responseFromPostgresError(e, "Create button failed");
  }
}
