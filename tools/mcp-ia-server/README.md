...
| **`unity_export_cell_chunk`** | **Bridge export sugar:** **`export_cell_chunk`** in one call — **`enqueueUnityBridgeJob`** + **`pollUnityBridgeJobUntilTerminal`** (**`unity_bridge_get`**). Params: **`origin_x`**, **`origin_y`**, **`chunk_width`**, **`chunk_height`**, optional **`timeout_ms`** (else **`BRIDGE_TIMEOUT_MS`** or **40000** default), optional **`agent_id`**. **Payload v2** (Postgres `editor_export_terrain_cell_chunk.document`): `cells[].{x, y, height, prefabName, waterBodyType, waterBodyId, waterMapBodyId, waterMapIsWater}` + `bodies[].{bodyId, classification, surfaceHeight, cellCount}`. `waterMapBodyId`/`waterMapIsWater` are WaterMap-authoritative (distinct from CityCell-cached `waterBodyId`/`waterBodyType` — orphan-cell detection); `bodies[]` exposes per-WaterBody surface heights without scanning cells. Prefer **`unity_bridge_command`** for other kinds. |
...
| **`theme_propose`** | LLM theme proposer stub (Stage 9.5 / TECH-15229). Returns hardcoded 8-token night-city dark palette (`ds-surface-*`, `ds-text-*`, `ds-accent-*`). Optional `mood` + `seed` reserved for real LLM integration. `stub: true` in response. |
| **`cron_audit_log_enqueue`** | Async cron enqueue — audit-log (TECH-18091). Fire-and-forget INSERT into `cron_audit_log_jobs`. Cron supervisor drains to `ia_master_plan_change_log`. Same payload shape as `master_plan_change_log_append`. Returns `{job_id, status:'queued'}` in < 100 ms. |
| **`cron_journal_append_enqueue`** | Async cron enqueue — journal-append (TECH-18091). Fire-and-forget INSERT into `cron_journal_append_jobs`. Cron supervisor drains to `ia_ship_stage_journal`. Same payload shape as `journal_append`. Returns `{job_id, status:'queued'}` in < 100 ms. |
...
| **`master_plan_lineage`** | DB-backed: return ordered version lineage for a slug as `[{version, parent_plan_slug, created_at, closed_at}, ...]` ASC. Walks `ia_master_plans` self-join via `parent_plan_slug` for slug + every ancestor and descendant in the chain. Input: `slug` (required). Returns empty array on unknown slug (partial-result, not throw). Used by `design-explore --resume` to compute `target_version = existing_max_version + 1`. ship-protocol Stage 5 / TECH-14103. |
...
| **`red_stage_proof_mine`** | Mine `Assets/Scripts/Tests/**` for C# test files matching BDD-name patterns for a given issue id. Returns `{ok:true, payload:{issue_id, candidate_length, candidate}}` — `candidate` is null when no match found. TECH-15910. |
...
| **`cron_anchor_reindex_enqueue`** | Fire-and-forget: enqueue one anchor reindex run into `cron_anchor_reindex_jobs`. Cron supervisor drains it by running `npm run generate:ia-indexes -- --write-anchors` (upserts `ia_spec_anchors` rows; cadence: `*/5 * * * *`). `paths`: changed spec file paths (informational). Returns `{job_id, status:'queued'}` in <100ms. TECH-18102. |
| **`cron_glossary_backlinks_enqueue`** | Fire-and-forget: enqueue one glossary back-link enrichment run into `cron_glossary_backlinks_jobs`. Cron supervisor drains it by running `node tools/scripts/glossary-backlink-enrich.mjs --plan-id {slug}` (cadence: `*/5 * * * *`). `slug`: master-plan slug. Returns `{job_id, status:'queued'}` in <100ms. TECH-18102. |
| **`cron_cache_warm_enqueue`** | Fire-and-forget: enqueue one cache-warm job into `cron_cache_warm_jobs`. Cron supervisor drains it by pre-populating `ia_mcp_context_cache` for the given `(cache_kind, cache_key)` pair (cadence: `*/5 * * * *`). For `db_read_batch`, `cache_key` format is `'db_read_batch:<raw_sql>'` — handler normalizes + hashes. Returns `{job_id, status:'queued'}` in <100ms. TECH-18105. |
| **`cron_cache_bust_enqueue`** | Fire-and-forget: enqueue one cache-bust invalidation into `cron_cache_bust_jobs`. Cron supervisor drains it by DELETEing `ia_mcp_context_cache` rows matching `key LIKE cache_key_pattern` (cadence: every minute). `cache_kind` informational; `cache_key_pattern` is a SQL LIKE pattern (e.g. `'db_read_batch:%'`). Returns `{job_id, status:'queued'}` in <100ms. TECH-18105. |
| **`cron_drift_lint_enqueue`** | Fire-and-forget: enqueue one drift-lint sweep into `cron_drift_lint_jobs`. Cron supervisor drains it by running `tools/scripts/drift-lint-sweep.mjs` (reads `ia_tasks` + `ia_spec_anchors`; cadence: every 10 min). `commit_sha` optional audit-trail SHA; `slug` reserved for future per-plan scoping. Returns `{job_id, status:'queued'}` in <100ms. TECH-18105. |
| **`ui_panel_get`** | Get one panel by slug: `panel_detail` row (rect_json, layout, padding_json, gap_px, params_json) joined to `catalog_entity` (id, kind, display_name, current_published_version_id) plus linked ui-calibration-corpus rows. Returns null payload when slug not found. |
| **`ui_panel_list`** | List all panels: slug, display_name, current_published_version_id, rect_json summary (anchor_min, anchor_max, size_delta only). Excludes retired panels by default. |
| **`ui_panel_publish`** | Publish a panel: increments `current_published_version_id` on `catalog_entity` and returns new version id. Flags snapshot regen required when `regen_snapshot=true` (default). Returns `{slug, prev_version_id, new_version_id, snapshot_regen_required}`. |
| **`ui_component_get`** | Get one component by slug: `catalog_entity` spine joined to `component_detail` (role, default_props_json, variants_json) plus linked panel consumers (panel slugs whose params_json references this component slug). Returns null payload when slug not found. |
| **`ui_component_list`** | List all components: slug, display_name, role, variants_json, current_published_version_id. Excludes retired components by default. |
| **`ui_component_publish`** | Publish a component: increments `entity_version` on `catalog_entity` and returns new version id. Flags components.json snapshot regen required when `regen_snapshot=true` (default). Returns `{slug, prev_version_id, new_version_id, new_version_number, snapshot_regen_required}`. |
| **`ui_token_get`** | Get one token by slug: `catalog_entity` spine joined to `token_detail` (token_kind, value_json, semantic_target_entity_id) plus linked panel consumers. Returns null payload when slug not found. |
| **`ui_token_list`** | List tokens: slug, display_name, token_kind, value_json summary, current_published_version_id. Filter by `token_kind` (color\|type-scale\|motion\|spacing\|semantic). Excludes retired tokens by default. |
| **`ui_token_publish`** | Publish a token: increments `entity_version` on `catalog_entity` and returns new version id. Flags tokens.json snapshot regen required when `regen_snapshot=true` (default). Returns `{slug, prev_version_id, new_version_id, new_version_number, snapshot_regen_required}`. |
