...
const r1 = await call("exit_play_mode", {});
console.log("[exit]", JSON.stringify({ ok: r1.ok, state: r1.response?.play_mode_state, err: r1.error }));
await new Promise(r => setTimeout(r, 3000));
const r2 = await call("get_play_mode_status", {});
console.log("[status]", JSON.stringify({ ok: r2.ok, state: r2.response?.play_mode_state, ready: r2.response?.ready }));
await pool.end();
