...
const r = await call("get_compilation_status", {}, 30000);
console.log(JSON.stringify({
  compiling: r.response?.compilation_status?.compiling,
  failed: r.response?.compilation_status?.compilation_failed,
  err: r.response?.compilation_status?.last_error_excerpt,
}, null, 2));
await pool.end();
