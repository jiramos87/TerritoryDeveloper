/**
 * MCP tool: project_spec_closeout_digest — structured extract from `.cursor/projects/{ISSUE_ID}.md` (TECH-58).
 */
...
const inputShape = {
  ...
  spec_path: z
    ...
    .optional()
    .describe(
      "Repo-relative path `.cursor/projects/{ISSUE_ID}.md`. Exactly one of `issue_id` or `spec_path` required.",
    ),
};
...
export function registerProjectSpecCloseoutDigest(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        "Parse a temporary project spec under `.cursor/projects/` for **project-spec-close**: H2 sections (Summary, Lessons Learned, Decision Log, …), cited issue ids, optional glossary_discover keywords, and heuristic G1–I1 checklist hints. Does not edit files or author normative spec prose. Requires exactly one of `issue_id` or `spec_path`.",
