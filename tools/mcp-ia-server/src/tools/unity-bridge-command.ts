...
> {
  ...
  try {
    let lastRow: BridgeRow | null = null;
    ...
    // Capture last_output_preview before deleting the pending row.
    const rawPreview = lastRow
      ? (lastRow.error ?? (lastRow.response ? JSON.stringify(lastRow.response) : ""))
      : "";
