...
// RoadStrokeTerrainRules (Territory.Utilities) lives in TerritoryDeveloper.Game — IsLandSlopeAllowedForRoadStroke inlined as private static below.

namespace Domains.Roads.Services
{
/// <summary>
/// Pure POCO service absorbing full interstate highway algorithm from Territory.Roads.InterstateManager.
/// No MonoBehaviour, no SerializeField. Holds all mutable algorithm state (positions, entry/exit, connectivity).
/// Cutover Stage 1.0 (TECH-26630) — Manager becomes single-line pass-through delegate.
/// Inv #2 (InvalidateRoadCache) + Inv #10 (PathTerraformPlan) preserved — triggered on Manager-side call sites.
/// </summary>
public class InterstateService
{
    // ------------------------------------------------------------------
    // Static direction helpers (carried from Stage 16 stub)
    // ------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------
    // Mutable algorithm state (owned by service, exposed via properties)
    // ------------------------------------------------------------------
    ...
    private const int InterstateGenSeed = 12345 + 100;
    ...
    /// <summary>No-dep ctor — valid for unit tests exercising pure static helpers (GetBorderIndex, IsCardinalStep, etc.) that need no grid/terrain/road.</summary>
    ...
    public bool GenerateAndPlaceInterstate(int attemptOffset = 0)
    {
        ...
        if (_interstatePositions.Count == 0 || _roads == null)
            return false;
    ...
    /// <summary>
    /// Try all border cells deterministically. Pick best valid path by cost.
    /// </summary>
    public bool TryGenerateInterstateDeterministic()
    {
        if (_roads == null) return false;
        var heightMap = _terrain != null ? _terrain.GetHeightMap() : null;
        if (heightMap == null || _grid == null) return false;
        ...
        candidates.Sort((a, b) =>
        {
            ...
            if (cmp != 0) return cmp;
            int scoreA = ComputeInterstateBorderEndpointScore(a.entry, w, h, heightMap)
                + ComputeInterstateBorderEndpointScore(a.exit, w, h, heightMap);
            int scoreB = ComputeInterstateBorderEndpointScore(b.entry, w, h, heightMap)
                + ComputeInterstateBorderEndpointScore(b.exit, w, h, heightMap);
    ...
    /// <summary>
    /// Generate interstate route + store positions. Retries until valid path exists.
    /// </summary>
    ...
    public void RebuildFromGrid()
    {
        ...
        for (int x = 0; x < w; x++)
        {
            for (int y = 0; y < h; y++)
            {
                ...
                if (cell != null && cell.isInterstate)
                    _interstatePositions.Add(new Vector2Int(x, y));
            }
        }
        ...
        foreach (Vector2Int pos in _interstatePositions)
        {
            int borderIdx = -1;
    ...
    {
        _isConnectedToInterstate = false;
        if (_grid == null || _interstatePositions.Count == 0) return;

        Queue<Vector2Int> queue = new Queue<Vector2Int>();
        HashSet<Vector2Int> visited = new HashSet<Vector2Int>();

        foreach (Vector2Int p in _interstatePositions)
        {
            queue.Enqueue(p);
            visited.Add(p);
        }
    ...
    private bool IsRoadAt(int gridX, int gridY)
    {
        ...
        return cell != null && cell.zoneType == Territory.Zones.Zone.ZoneType.Road;
    }
    ...
    private List<int> GetBordersWithLand(int w, int h, HeightMap heightMap)
    {
    ...
    private bool HasAnyValidCellOnBorder(int border, int w, int h, HeightMap heightMap)
    {
        switch (border)
        {
            case 0:
    ...
    private bool IsCellAllowedForInterstate(int x, int y, int w, int h, HeightMap heightMap, bool checkSlopes)
    {
    ...
    private List<Vector2Int> GetValidBorderCells(int border, int w, int h, HeightMap heightMap)
    {
    ...
    private int ComputeInterstateBorderEndpointScore(Vector2Int c, int w, int h, HeightMap heightMap)
    {
    ...
    private List<Vector2Int> FindInterstatePathAStar(Vector2Int start, Vector2Int end, int w, int h, HeightMap heightMap)
    {
        ...
        for (int attempt = 0; attempt < maxParallelOffsetAttempts; attempt++)
        {
            ...
            if (!IsCellAllowedForInterstate(offStart.x, offStart.y, w, h, heightMap, checkSlopes: true)) continue;
            if (!IsCellAllowedForInterstate(offEnd.x, offEnd.y, w, h, heightMap, checkSlopes: true)) continue;
    ...
    private bool IsDirectStepValid(Vector2Int from, Vector2Int to, int w, int h, HeightMap heightMap, Vector2Int pathEnd)
    {
        ...
        while (open.Count > 0 && explored < maxNodes)
        {
            ...
            foreach (Vector2Int neighbor in neighbors)
            {
                ...
                bool allowed = cellHeight > 0
                    ? IsCellAllowedForInterstate(neighbor.x, neighbor.y, w, h, heightMap, checkSlopes: true)
    ...
    private List<Vector2Int> BiasedWalkPath(Vector2Int start, Vector2Int end, int w, int h, HeightMap heightMap)
    {
        ...
        while (current != end && steps < maxSteps)
        {
            ...
            foreach (Vector2Int c in candidates)
            {
                ...
                bool allowed = false;
                if (cellHeight > 0)
                    allowed = IsCellAllowedForInterstate(c.x, c.y, w, h, heightMap, checkSlopes: true);
    ...
    /// Allowed: Flat + cardinal ramps (N/S/E/W) + corner-up concave cells. Outer-corner diagonal slopes disallowed.
    /// </summary>
    private static bool IsLandSlopeAllowedForRoadStroke(TerrainSlopeType slopeType)
    {
