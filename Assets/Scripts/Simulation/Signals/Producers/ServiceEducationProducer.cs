...
{
    /// <summary>
    /// Stage 9.A producer emitting <see cref="SignalTuningWeightsAsset.ServiceEducationCoverage"/>
    /// into <see cref="SimulationSignal.ServiceEducation"/> at education-equipped state-service
    /// cells only. Predicate: <c>cell.zoneType ∈ {StateServiceLight,Medium,Heavy}Building</c> AND
    /// <c>cell.occupiedBuilding != null</c> AND <c>Zone.SubTypeId == 2</c> (Education = id 2 per
    ...
    /// </summary>
    public class ServiceEducationProducer : MonoBehaviour, ISignalProducer
    {
        ...
        private void Awake()
        {
            ...
            if (tuningWeights == null)
            {
                ...
                if (tuningWeights == null)
                {
                    Debug.LogError("ServiceEducationProducer.tuningWeights unresolved — SignalTuningWeightsAsset missing under Resources/ and no Inspector wiring. EmitSignals will no-op.");
                }
            }
        }

        /// <summary>Iterate grid; write <c>ServiceEducationCoverage</c> at education-equipped state-service cells only.</summary>
        public void EmitSignals(SignalFieldRegistry registry)
        {
            ...
            SignalField field = registry.GetField(SimulationSignal.ServiceEducation);
            if (field == null)
            {
                return;
            }

            float coverage = tuningWeights.ServiceEducationCoverage;
