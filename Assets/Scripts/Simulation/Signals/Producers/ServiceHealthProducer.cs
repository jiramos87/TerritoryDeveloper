...
{
    /// <summary>
    /// Stage 9.A producer emitting <see cref="SignalTuningWeightsAsset.ServiceHealthCoverage"/>
    /// into <see cref="SimulationSignal.ServiceHealth"/> at health-equipped state-service cells
    /// only. Predicate: <c>cell.zoneType ∈ {StateServiceLight,Medium,Heavy}Building</c> AND
    /// <c>cell.occupiedBuilding != null</c> AND <c>Zone.SubTypeId == 3</c> (Health = id 3 per
    ...
    /// </summary>
    public class ServiceHealthProducer : MonoBehaviour, ISignalProducer
    {
        ...
        private void Awake()
        {
            ...
            if (tuningWeights == null)
            {
                ...
                if (tuningWeights == null)
                {
                    Debug.LogError("ServiceHealthProducer.tuningWeights unresolved — SignalTuningWeightsAsset missing under Resources/ and no Inspector wiring. EmitSignals will no-op.");
                }
            }
        }

        /// <summary>Iterate grid; write <c>ServiceHealthCoverage</c> at health-equipped state-service cells only.</summary>
        public void EmitSignals(SignalFieldRegistry registry)
        {
            ...
            SignalField field = registry.GetField(SimulationSignal.ServiceHealth);
            if (field == null)
            {
                return;
            }

            float coverage = tuningWeights.ServiceHealthCoverage;
