...
{
    /// <summary>
    /// Stage 9.A consumer reading <see cref="SimulationSignal.ServiceEducation"/> per-district mean
    ...
    /// </summary>
    public class ServiceEducationConsumer : MonoBehaviour, ISignalConsumer
    {
        ...
        private void Awake()
        {
            if (tuningWeights == null)
            {
                ...
                if (tuningWeights == null)
                {
                    Debug.LogError("ServiceEducationConsumer.tuningWeights unresolved — SignalTuningWeightsAsset missing under Resources/ and no Inspector wiring. ConsumeSignals will no-op.");
                }
            }
        }

        /// <summary>Clear + refill <see cref="LastTickMeanByDistrict"/> from <paramref name="cache"/> for <see cref="SimulationSignal.ServiceEducation"/>; iterates <c>0..DistrictMap.DistrictCount-1</c>.</summary>
        public void ConsumeSignals(SignalFieldRegistry registry, DistrictSignalCache cache)
        {
            ...
            {
                float v = cache.Get(districtId, SimulationSignal.ServiceEducation);
                if (float.IsNaN(v))
                {
                    continue;
                }
