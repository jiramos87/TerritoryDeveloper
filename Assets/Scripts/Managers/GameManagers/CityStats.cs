...
public class CityStats : MonoBehaviour, ICityStats, ICityStatsAuto
{
    ...
    [SerializeField][FormerlySerializedAs("happiness")] private float _happiness = StartingHappiness;
    ...
    const float MaxHappiness = 100f, StartingHappiness = 50f;
    ...
    public float GetNormalizedHappiness() => Mathf.Clamp01(happiness / MaxHappiness);
    public float GetHappinessDemandMultiplier() { float n = Mathf.Clamp01(ComputeTargetHappiness() / MaxHappiness); return DemMul0 + n * (DemMul1 - DemMul0); }
    float ComputeTargetHappiness() { float empl = _employmentManager != null ? _employmentManager.GetEmploymentRate() / MaxHappiness : 0.5f; float tax = 0f; if (_economyManager != null) { float mx = Mathf.Max(_economyManager.residentialIncomeTax, Mathf.Max(_economyManager.commercialIncomeTax, _economyManager.industrialIncomeTax)); if (mx > ComfTax) tax = -Mathf.Clamp01((mx - ComfTax) / (MaxTax - ComfTax)); } float dev = (residentialZoneCount + commercialZoneCount + industrialZoneCount) > 0 ? Mathf.Clamp01((float)(residentialBuildingCount + commercialBuildingCount + industrialBuildingCount) / (residentialZoneCount + commercialZoneCount + industrialZoneCount)) : 0f; return Mathf.Clamp(HB + empl * WE + tax * happinessWeightTax + Mathf.Clamp01(happinessServiceCoverageStub) * WS + Mathf.Clamp01(GetForestHappinessBonus() / MaxForest) * WF + dev * happinessWeightDevelopment - Mathf.Clamp01(pollution / 200f) * WP, 0f, MaxHappiness); }
